...
// ===== Config =====
static constexpr const char* VERSION = "0.64.01";
...
static std::string g_audioDriver;
...
static bool g_planetMode = true;
static float g_planetYaw = 0.0f;
static float g_planetPitch = 0.24f;
static float g_planetZoom = 1.0f;
static float g_planetSpin = 0.36f;
static float g_planetRadiusScale = 0.24f;
...
struct SynthOut {
  ...
  bool open(const std::string& sf2, float g, const std::string& driver, const std::string& device){
    ...
    // Strong stability bias: larger buffers, simpler DSP.
    ...
    fluid_settings_setint(settings, "synth.polyphony", 96);
    ...
    fluid_settings_setnum(settings, "synth.reverb.level", 0.25);
    fluid_settings_setint(settings, "synth.chorus.active", 0);
    fluid_settings_setint(settings, "synth.chorus.nr", 1);
    fluid_settings_setnum(settings, "synth.chorus.level", 0.3);
    fluid_settings_setnum(settings, "synth.chorus.speed", 0.16);
    fluid_settings_setnum(settings, "synth.chorus.depth", 4.0);
...
static void stepWater(World& w, Rng& r){
  ...
  for (int t=0; t<simThreads; ++t){
    for (const auto& sp : spills[t]){
      ...
      int owner = std::min(simThreads-1, ny / chunk);
      int lny = ny - w.waterDeltaY0[owner];
      if (owner >= 0 && owner < simThreads && lny >= 0 && lny < (int)deltas[owner].size()) {
...
static inline void queuedNoteOn(SynthOut& synth, MidiOut& midi, int ch, int note, int vel, int tick, int dur){
  ...
  off.ch = ch;
  off.note = note;
  off.offTick = tick + std::max(1, dur);
...
static inline RGB gradeColor(RGB c){
  ...
  bf = avg + (bf - avg) * sat;
  auto adj = [&](float f){
    f = (f - 0.5f) * (1.30f * g_gradeContrast) + 0.5f; // contrast
    f += g_gradeLift;
    f *= 0.72f; // darken
...
static inline Vec3 norm3(const Vec3& v){
  float m = std::sqrt(std::max(1e-8f, v.x*v.x + v.y*v.y + v.z*v.z));
  return { v.x/m, v.y/m, v.z/m };
}

static inline float planetRadiusPx(const Layout& L){
  return g_planetRadiusScale * (float)std::min(L.screenW, L.simHpx) * std::clamp(g_planetZoom, 0.45f, 2.4f);
}

static inline bool planetScreenToWorld(const Layout& L, int sx, int sy, int& wx, int& wy){
  float cx = 0.5f * (float)L.screenW;
  float cy = 0.52f * (float)L.simHpx;
  float radius = planetRadiusPx(L);
  if (radius < 20.f) return false;
  float nx = ((float)sx - cx) / radius;
  float ny = ((float)sy - cy) / radius;
  float r2 = nx*nx + ny*ny;
  if (r2 > 1.f) return false;
  float nz = std::sqrt(std::max(0.f, 1.f - r2));
  Vec3 camN{ nx, -ny, nz };
  // inverse camera-space rotations back to world-space sphere normal.
  Vec3 worldN = rotY(rotX(camN, -g_planetPitch), -g_planetYaw);
  worldN = norm3(worldN);
  float lon = std::atan2(worldN.z, worldN.x);
  if (lon < 0.f) lon += 6.2831853f;
  float lat = std::asin(std::clamp(worldN.y, -1.f, 1.f));
  wx = clampi((int)std::lround((lon / 6.2831853f) * (float)(W-1)), 0, W-1);
  wy = clampi((int)std::lround(((lat + 1.5707963f) / 3.1415926f) * (float)(H-1)), 0, H-1);
  return true;
}

static void renderPlanetWorld(SDL_Renderer* ren, const Layout& L, World& w, int tick){
  float cx = 0.5f * (float)L.screenW;
  float cy = 0.52f * (float)L.simHpx;
  float radius = planetRadiusPx(L);
  if (radius < 20.f) return;

  struct PV { int sx=0, sy=0; float z=-1.f; RGB col{0,0,0}; bool vis=false; };
  int LAT = std::clamp((int)std::lround(radius * 0.70f), 96, 220);
  if (g_wantSynth) LAT = std::max(80, LAT - 20);
  int LON = std::clamp(LAT * 2, 192, 440);
  std::vector<PV> v((LAT+1)*(LON+1));
  auto id = [&](int iy,int ix){ return iy*(LON+1) + ix; };

  Vec3 light = norm3({-0.40f, 0.70f, 0.55f});
  Season s = seasonAt(tick);
  for (int iy=0; iy<=LAT; ++iy){
    float lat = -1.5707963f + 3.1415926f * ((float)iy / (float)LAT);
    float cLat = std::cos(lat);
    float sLat = std::sin(lat);
    for (int ix=0; ix<=LON; ++ix){
      float lon = 6.2831853f * ((float)ix / (float)LON);
      float cLon = std::cos(lon);
      float sLon = std::sin(lon);
      Vec3 n{ cLat*cLon, sLat, cLat*sLon };
      Vec3 r = rotX(rotY(n, g_planetYaw), g_planetPitch);

      int k = id(iy,ix);
      if (r.z <= -0.18f) { v[k].vis=false; continue; }
      int wx = clampi((int)std::lround((lon / 6.2831853f) * (float)(W-1)), 0, W-1);
      int wy = clampi((int)std::lround(((lat + 1.5707963f) / 3.1415926f) * (float)(H-1)), 0, H-1);
      char c = renderCharAt(w, wx, wy, tick);
      if (w.water[wy][wx] <= 0.2f && c == w.terrain[wy][wx]) {
        c = terrainGlyphVariant(c, hash3((uint32_t)wx,(uint32_t)wy,(uint32_t)(tick/7)), s, w.weather);
      }
      RGB col = gradeColor(fgForChar(w, c, wx, wy, s));
      float ndl = std::max(0.f, dot3(norm3(r), light));
      float rim = std::pow(std::clamp(1.0f - std::max(0.f, r.z), 0.f, 1.f), 1.8f);
      float shade = 0.22f + 0.74f*ndl + 0.12f*rim;
      if (w.weather.state==STORM) shade *= 0.90f;
      col.r = (uint8_t)clampi((int)std::lround((float)col.r * shade), 0, 255);
      col.g = (uint8_t)clampi((int)std::lround((float)col.g * shade), 0, 255);
      col.b = (uint8_t)clampi((int)std::lround((float)col.b * shade), 0, 255);

      // Orthographic projection to keep picking stable and avoid warped artifacts.
      v[k].sx = (int)std::lround(cx + r.x * radius);
      v[k].sy = (int)std::lround(cy - r.y * radius);
      v[k].z = r.z;
      v[k].col = col;
      v[k].vis = true;
    }
  ...
  // Starfield backdrop for space context.
  SDL_SetRenderDrawBlendMode(ren, SDL_BLENDMODE_BLEND);
  for (int i=0; i<260; ++i){
    int x = (i*53 + tick*2) % std::max(1, L.screenW);
    int y = (i*97 + tick) % std::max(1, L.screenH);
    uint8_t a = (uint8_t)(10 + ((i*37 + tick) & 15));
    setColor(ren, 160,180,220, a);
    SDL_Rect p{ x, y, 1, 1 };
    SDL_RenderFillRect(ren, &p);
  }

  // Draw far-to-near sample points so surface reads as a textured planet.
  std::vector<int> order;
  order.reserve(v.size());
  for (int i=0; i<(int)v.size(); ++i) if (v[i].vis) order.push_back(i);
  std::sort(order.begin(), order.end(), [&](int a, int b){ return v[a].z < v[b].z; });
  for (int idx : order){
    const PV& p = v[idx];
    int sz = (p.z > 0.65f) ? 2 : 1;
    SDL_Rect rc{ p.sx - sz/2, p.sy - sz/2, sz, sz };
    setColor(ren, p.col.r, p.col.g, p.col.b, 235);
    SDL_RenderFillRect(ren, &rc);
  }

  // Low-poly wireframe edges.
  SDL_SetRenderDrawBlendMode(ren, SDL_BLENDMODE_BLEND);
  for (int iy=0; iy<LAT; ++iy){
    for (int ix=0; ix<LON; ++ix){
      const PV& a = v[id(iy,ix)];
      const PV& b = v[id(iy,ix+1)];
      const PV& c = v[id(iy+1,ix)];
      if (a.vis && b.vis && a.z>-0.04f && b.z>-0.04f){
        setColor(ren, 24,32,44, (uint8_t)clampi((int)std::lround(64.f + 64.f*std::max(a.z,b.z)), 36, 120));
        SDL_RenderDrawLine(ren, a.sx, a.sy, b.sx, b.sy);
      }
      if (a.vis && c.vis && a.z>-0.04f && c.z>-0.04f){
        setColor(ren, 24,32,44, (uint8_t)clampi((int)std::lround(64.f + 64.f*std::max(a.z,c.z)), 36, 120));
        SDL_RenderDrawLine(ren, a.sx, a.sy, c.sx, c.sy);
      }
    }
  }
  ...
  for (int ring=0; ring<8; ++ring){
    float rr = radius + (float)ring * 1.4f;
    uint8_t a = (uint8_t)clampi(26 - ring*3, 0, 26);
    setColor(ren, 70,120,190, a);
    for (int d=0; d<360; d+=2){
      float rad = (float)d * 0.0174532925f;
      int x = (int)std::lround(cx + std::cos(rad) * rr);
      int y = (int)std::lround(cy + std::sin(rad) * rr);
...
static void render(SDL_Renderer* ren, const Layout& L, World& w, GlyphCache& gcWorld, GlyphCache& gcText, int tick, bool showMenu, int menuPage, int menuSel){
  ...
  if (g_hoverInspect && mouseInSim) {
    if (g_planetMode) {
      if (planetScreenToWorld(L, mouseX, mouseY, hoverWX, hoverWY)) {
  ...
  if (g_planetMode) {
    renderPlanetWorld(ren, L, w, tick);
  } else for(int y=0;y<viewH;++y){
  ...
  if (!showMenu) {
    if (g_planetMode) {
      std::snprintf(buf, sizeof(buf), "M menu  V flat/planet  B biome  R reseed  [ ] speed  WASD rotate  Z/X globe zoom  UP/DOWN tilt  ,/. spin  G ghibli  H modhot  I inspect  RMB pin  F5 preset  F6 save  F7 midi  F9 audio  F11 fullscreen  %s", VERSION);
    } else {
      std::snprintf(buf, sizeof(buf), "M menu  V flat/planet  B biome  R reseed  [ ] speed  WASD pan  Z/X zoom  G ghibli  H modhot  I inspect  RMB pin  F5 preset  F6 save  F7 midi  F9 audio  F11 fullscreen  %s", VERSION);
    }
  ...
  if (showMenu) {
    ...
    if (menuPage==0) {
      ...
      ty += 12;
      std::snprintf(buf, sizeof(buf), "Render: %s", g_planetMode ? "PLANET 3D" : "FLAT 2D");
...
int main(int argc, char** argv){
  ...
  while(running){
    ...
    SDL_Event e; while(SDL_PollEvent(&e)){
      ...
      if (e.type==SDL_KEYDOWN){
        switch(e.key.keysym.sym){
          ...
          case SDLK_w:
            if (!showMenu) {
              if (g_planetMode) g_planetPitch = std::clamp(g_planetPitch + 0.06f, -1.1f, 1.1f);
              else { g_camY -= std::max(1, zoomViewH()/10); clampCameraToZoom(); }
            }
            break;
          case SDLK_s:
            if (!showMenu) {
          ...
          case SDLK_a:
            if (!showMenu) {
              if (g_planetMode) g_planetYaw -= 0.09f;
              else { g_camX -= std::max(1, zoomViewW()/10); clampCameraToZoom(); }
            }
            break;
          case SDLK_d:
            if (!showMenu) {
          ...
          case SDLK_z:
            if (!showMenu) {
              if (g_planetMode) g_planetZoom = std::clamp(g_planetZoom + 0.08f, 0.45f, 2.4f);
              else { g_zoom = clampi(g_zoom + 1, 1, 12); clampCameraToZoom(); }
            }
            break;
          case SDLK_x:
            if (!showMenu) {
            ...
            break;
          case SDLK_v:
            if (!showMenu) g_planetMode = !g_planetMode;
            break;
          case SDLK_COMMA:
            if (!showMenu && g_planetMode) g_planetSpin = std::clamp(g_planetSpin - 0.04f, -1.5f, 1.5f);
            break;
          case SDLK_PERIOD:
            if (!showMenu && g_planetMode) g_planetSpin = std::clamp(g_planetSpin + 0.04f, -1.5f, 1.5f);
            break;
          ...
          case SDLK_UP: {
            ...
            else if (g_planetMode) { g_planetPitch = std::clamp(g_planetPitch + 0.04f, -1.1f, 1.1f); }
          } break;
          case SDLK_DOWN: {
      ...
      if (e.type==SDL_MOUSEBUTTONDOWN) {
        ...
        int wx = -1, wy = -1;
        if (g_planetMode) {
          if (!planetScreenToWorld(layout, mx, my, wx, wy)) continue;
        } else {
          float camRX = g_camFX;
          float camRY = g_camFY;
          float zoomR = std::clamp(g_zoomF, 1.0f, 12.0f);
          ...
          wx = clampi(camX0 + sx, 0, W-1);
          wy = clampi(camY0 + sy, 0, H-1);
        }
    ...
    if (g_planetMode) {
      float autoSpin = g_planetSpin + 0.18f*pulse + 0.12f*storm;
      g_planetYaw += autoSpin * frameDt;
      if (g_planetYaw > 6.2831853f) g_planetYaw -= 6.2831853f;
      if (g_planetYaw < -6.2831853f) g_planetYaw += 6.2831853f;
    }
    ...
    int usedMs = (int)std::chrono::duration_cast<std::chrono::milliseconds>(frameEnd - frameStart).count();
    if (usedMs < targetFrameMs) SDL_Delay((Uint32)(targetFrameMs - usedMs));
  }
