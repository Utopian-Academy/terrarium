...
// ===== Forward declarations for new features (declared later) =====
struct World;
struct Rng;
struct Ripple;
struct AleaWeights;
...
extern int g_camX;
extern int g_camY;
extern std::vector<Ripple> g_ripples;
extern AleaWeights g_alea;
...
static inline char renderCharAt(const World& w, int x, int y, int tick) {
  ...
  for (const auto& r : g_ripples) {
    float rx = float(x - r.cx), ry = float(y - r.cy);
...
// ===== Camera / zoom + ripples + per-voice settings =====
int g_zoom = 1;         // 1..4
int g_camX = 0;         // top-left world cell of view
int g_camY = 0;

struct Ripple {
  int cx=0, cy=0;
  float t=0.f;      // seconds
  float amp=3.0f;   // max cell offset
  float speed=18.f; // cells/sec
  float width=2.5f; // ring thickness in cells
  float chaos=0.6f; // how strongly to perturb sim
};
std::vector<Ripple> g_ripples;

struct AleaWeights {
  float chaos = 1.0f;       // global multiplier
  float rainChance = 1.0f;  // affects weather transitions and rainfall overlays
  float spawnChance = 1.0f; // affects some rare events (rainbows, big-life etc)
  float mutationRate = 1.0f;// reserved hook (agents/plants)
  float drift = 1.0f;       // reserved hook (wind/water drift)
};
AleaWeights g_alea;
...
static void render(SDL_Renderer* ren, const Layout& L, World& w, GlyphCache& gcWorld, GlyphCache& gcText, int tick, bool showMenu, int menuPage, const std::vector<MidiParam>& params, int menuSel, bool midiEnabled, bool midiClockOut, bool useSimClock, int rootKey, ScaleType scaleType, bool synthEnabled, const std::string& sf2Path, UiLang uiLang) {
  ...
  for (int y=0; y<viewH; ++y) {
    ...
    for (int x=0; x<viewW; ++x) {
      int wx = g_camX + x;
      int x0 = (x * L.simWpx) / viewW;
      int x1 = ((x+1) * L.simWpx) / viewW;
...
if (showMenu) {
  ...
  if ((menuPage%5)==0) {
    ...
    auto gmName = [&](int p)->const char*{
      // minimal labels (still works if SF2 differs)
      switch(p){
        case 10: return "MusicBox";
        case 11: return "Vibraphone";
        case 12: return "Marimba";
        case 14: return "TubularBells";
        case 15: return "Dulcimer";
        case 98: return "Crystal";
        case 102:return "EchoDrops";
        default: return "";
      }
    };
...
case SDLK_TAB: if (showMenu) menuPage = (menuPage + 1) % 5; break;
...
case SDLK_UP: {
            int maxSel = 0;
            if (showMenu) {
              if ((menuPage%5)==0) maxSel = (int)params.size();
              else if ((menuPage%5)==1) maxSel = 5;
              else if ((menuPage%5)==4) maxSel = NUM_VOICES*3;
              else maxSel = 1;
...
case SDLK_MINUS:
case SDLK_KP_MINUS: {
  if (showMenu && (menuPage%5)==1) {
    float* rows[] = { &g_alea.chaos, &g_alea.rainChance, &g_alea.spawnChance, &g_alea.mutationRate, &g_alea.drift };
    int sel = clampi(menuSel,0,4);
    *rows[sel] = std::max(0.0f, *rows[sel] - 0.05f);
  } else if (showMenu && (menuPage%5)==4) {
...
case SDLK_KP_PLUS: {
  if (showMenu && (menuPage%5)==1) {
    float* rows[] = { &g_alea.chaos, &g_alea.rainChance, &g_alea.spawnChance, &g_alea.mutationRate, &g_alea.drift };
    int sel = clampi(menuSel,0,4);
    *rows[sel] = std::min(2.0f, *rows[sel] + 0.05f);
  } else if (showMenu && (menuPage%5)==4) {
    int sel = clampi(menuSel,0,NUM_VOICES*3-1);
    int v = sel/3; int f = sel%3;
...
case SDLK_s: if (g_zoom>1 && !showMenu) { g_camY += 2; clampCameraToWorld(); break; }
          scaleType = (ScaleType)(((int)scaleType + 1) % 5); break;
default: break;
        }
      }
