...
static void applyAutomation(SynthOut& synth, MidiOut& midi, int tick){
  for(int ch=0; ch<4; ++ch){
    if (g_voiceProgDirty[ch] && (tick - g_progLastTick[ch] > 8)) {
...
static RGB fgForChar(const World& w, char c, int x, int y, Season s){
  ...
  if (c>='1' && c<='7') {
    ...
    return boostColor({ (uint8_t)r, (uint8_t)g, (uint8_t)bl }, 1.35f, 1.20f);
  }
  if (c=='=') return boostColor(b.foam, 1.25f, 1.25f);
  if (c=='d' || c=='e' || c=='g') return boostColor(jitter({110,80,55}, 4), 1.15f, 1.10f);
  if (c=='^') return boostColor(b.rock, 1.15f, 1.10f);
  if (c=='.') return boostColor(jitter(b.soil, 3), 1.20f, 1.10f);
  if (c=='s') return boostColor(jitter({150,120,80}, 4), 1.20f, 1.10f);
  if (c==',' || c=='"' || c==';' || c=='#') return boostColor(jitter(b.grass, 4), 1.30f, 1.15f);
  if (c=='T' || c=='Y') return boostColor(jitter(b.tree, 3), 1.25f, 1.10f);
  if (c=='m') return boostColor({40,230,150}, 1.45f, 1.25f);
  if (c=='a') return boostColor({30,200,190}, 1.45f, 1.25f);
  if (c=='t') return boostColor({220,170,100}, 1.20f, 1.10f);
  if (c=='l') return boostColor({140,210,160}, 1.20f, 1.10f);
  if (c=='n') return boostColor({90,230,140}, 1.35f, 1.20f);
  if (c=='q') return boostColor({200,90,255}, 1.35f, 1.25f);
  if (c=='f' || c=='+' || c=='&' || c=='!') {
  ...
  if (std::strchr("rdgfcwpbevAZ", c)) {
    if (c=='r') return boostColor({210,160,120}, 1.35f, 1.15f); // rabbit
    if (c=='d') return boostColor({200,150,95}, 1.2f, 1.10f);  // deer
    if (c=='g') return boostColor({170,170,150}, 1.2f, 1.10f); // goat
    if (c=='f') return boostColor({90,220,255}, 1.45f, 1.25f);  // fish
    if (c=='c') return boostColor({230,140,90}, 1.35f, 1.15f);  // crab
    if (c=='p') return boostColor({120,230,150}, 1.35f, 1.15f); // frog
    if (c=='w') return boostColor({190,190,210}, 1.15f, 1.10f); // wolf
    if (c=='b') return boostColor({150,120,90}, 1.2f, 1.10f);  // bear
    if (c=='e') return boostColor({70,180,225}, 1.3f, 1.15f);  // eel
    if (c=='v') return boostColor({210,210,150}, 1.3f, 1.10f); // bird
    if (c=='A' || c=='Z') return boostColor({220,90,240}, 1.4f, 1.20f);
    return {220,220,220};
  }
  return {200,200,200};
}
