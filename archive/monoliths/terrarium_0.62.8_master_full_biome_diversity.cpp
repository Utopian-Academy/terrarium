...
static constexpr int MAX_FLORA  = 250;
...
static void spawnInitialAgents(World& w, Rng& r){
  ...
  // Start near-zero; let the world spawn them via rare events.
  const int START_INIT_AGENTS = 2;
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.62.9 | biome ") + biomeName(biome) +
