                                 ...
                                 const std::vector<MidiParam>& params)
{
  ...
  // "Instr" (0..1) explicitly selects from the palette.
  // Otherwise, we do *fast* auto-switching (every note) driven by WIND.
  static int currentProgram = -1;

  auto chooseFromPalette = [&](float x01)->int {
    // Curated "magical marimba/musicbox" palette (GM-ish numbers; depends on SF2)
    // 10 Music Box, 11 Vibraphone, 12 Marimba, 14 Tubular Bells, 98 Crystal, 102 Echo Drops
  ...
  // If the Instr knob has moved from near-zero, treat it as an explicit selection.
  float instr01 = getParam01("Instr", 0.0f);
  bool instrExplicit = instr01 > 0.01f;

  // Choose an instrument program. If explicit, it's stable. If not, we will switch quickly.
  auto pickProgramFastFromWind = [&]()->int {
    // One data source: WIND (strength + direction), plus tick to evolve.
    const float s01 = (MAX_WIND > 0) ? (float)world.wind.strength / (float)MAX_WIND : 0.0f;
    // Hash in direction and time so it can flicker between palette entries.
    uint32_t h = hash3((uint32_t)(world.wind.dx + 3), (uint32_t)(world.wind.dy + 3), (uint32_t)(tick / 3));
    float j01 = (float)(h & 0xFFFFu) / 65535.0f;
    // Stronger wind biases toward brighter/shimmery presets; calm biases toward music box/marimba.
    float pick = std::clamp(0.20f * (1.0f - s01) + 0.80f * j01, 0.0f, 0.9999f);
    return chooseFromPalette(pick);
  };

  auto applyProgram = [&](int prog){
    if (prog == currentProgram) return;
#ifdef USE_FLUIDSYNTH
    if (synth.synth) fluid_synth_program_change(synth.synth, 0, prog);
#endif
    currentProgram = prog;
  };

  if (instrExplicit) {
    applyProgram(chooseFromPalette(instr01));
  } else if (currentProgram < 0) {
    applyProgram(chooseFromPalette(0.10f));
  }

  // --- Note generator ---
  // Evaluate every N ticks to avoid constant retriggering.
  const int N = 9; // fast: switch/trigger about ~6-7 times/sec at 60 TPS
  if ((tick % N) != 0) return;

  // Fast auto-instrument switching: change program at each note boundary if not explicitly controlled.
  if (!instrExplicit) {
    applyProgram(pickProgramFastFromWind());
  }

  // Release previous held notes.
  if (heldNote  >= 0) synth.noteOff(0, heldNote, 0);
  if (heldNote2 >= 0) synth.noteOff(0, heldNote2, 0);
  heldNote = heldNote2 = -1;

  int waterTiles = 0, plants = 0, predators = 0, whales = 0;
  ...
  // Center pitch wanders around a target; avoids monotonic chromatic climbing.
  static float center = 62.0f;
  float target = 60.0f + (w01 - 0.35f) * 10.0f + (p01 - 0.20f) * 4.0f;
  // Storms push slightly higher and brighter.
  if (world.weather.state == STORM) target += 3.0f;
  // Whales push a gentle octave lift occasionally.
  if (whales > 0) target += 2.0f;

  center = center * 0.985f + target * 0.015f;

  // Choose a chord tone in a small range around center.
  int spread = 7;
  int raw1 = (int)std::lround(center) + r.irange(-spread, spread);
  // Occasional octave sparkle.
  if (r.oneIn(10)) raw1 += 12;

  // Second note is rarer: a fifth / third-ish offset with some spice.
  int raw2 = raw1 + (r.oneIn(2) ? 7 : 4) + r.irange(-1, 1);
  if (predators > 0 && r.oneIn(3)) raw2 -= 5; // darker tension when predators around

  int n1 = quantizeNoteToScale(raw1, rootKey, scaleType);
  int n2 = quantizeNoteToScale(raw2, rootKey, scaleType);

  heldNote  = std::clamp(n1, 36, 96);
  heldNote2 = std::clamp(n2, 36, 96);

  // Velocity: soft and delicate; slightly stronger in storms.
  int v1 = 52 + (int)(p01 * 30.0f);
  if (world.weather.state == STORM) v1 += 18;
  v1 = std::clamp(v1, 35, 95);

  synth.noteOn(0, heldNote, v1);

  // Occasional gentle harmony
  if (!r.oneIn(4)) {
    int v2 = std::clamp(v1 - 14, 25, 80);
    synth.noteOn(0, heldNote2, v2);
  }

  // Sparse percussion accents on channel 9 (keep very rare so it stays "magical")
  if (world.weather.state == STORM && r.oneIn(12)) synth.noteOn(9, 80, 70); // mute triangle-ish
  if (world.weather.state == RAIN  && r.oneIn(18)) synth.noteOn(9, 81, 55); // open triangle-ish
