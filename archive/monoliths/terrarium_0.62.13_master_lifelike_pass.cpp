...
static const char* g_modName[MOD_N] = {
  ...
  "cloud_shadow", "oddity_1", "oddity_2", "oddity_3", "oddity_4",
...
static void updateModPool(World& w, int tick, int viewW, int viewH){
  ...
  float o0 = cloudShadow;                                  // cloud shadow strength
  ...
  float o4 = g_modVal[21] * (0.3f + g_modVal[2]);          // bird cohesion * overlay
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.62.14 | biome ") + biomeName(biome) +
