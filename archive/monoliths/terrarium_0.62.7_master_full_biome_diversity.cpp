...
if (showMenu) {
  ...
  snprintf(buf, sizeof(buf),
           "[TAB] Page:%s   Biome:%s   Season:%s   Wind:(%d,%d) s=%d   Weather:%d   Water:%d%%   Agents:%d/%d   Flora:%d/%d   Synth:%s",
  ...
  if ((menuPage%9)==0) {
    ...
    ty += 10*L.scale;
    snprintf(buf, sizeof(buf), "Fauna (agents): %d / %d", (int)w.agents.size(), MAX_AGENTS);
    drawString(ren, gcText, tx, ty, buf, 220,220,220, 230, L.scale);
    ty += 10*L.scale;
    snprintf(buf, sizeof(buf), "Flora (glyphs): %d / %d", floraGlyphs, MAX_FLORA);
    drawString(ren, gcText, tx, ty, buf, 220,220,220, 230, L.scale);
    ty += 10*L.scale;
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.62.8 | biome ") + biomeName(biome) +
