...
static void stepEntities(World& w, Rng& r, Season s, int tick) {
  ...
  int owls = countEnt('O');
  int deer = countEnt('D');
int fish = countEnt('>') + countEnt('<');
int cranes = countEnt('C');
...
// Fish: live in water (depth>=2). Prefer kelp tiles as cover.
if (fish < FISH_CAP && r.oneIn(10)) {
  for (int tries=0; tries<4000; ++tries) {
    int x=r.i(1,W-2), y=r.i(1,H-2);
    if (cur[y][x] != ' ') continue;
    if (w.water[y][x] < 2) continue;
    if (w.terrain[y][x] == '*') continue;
    // bias toward kelp / wetlands
    int bias = (w.terrain[y][x]==KELP_GLYPH) ? 5 : 0;
    if (bias==0 && !r.oneIn(3)) continue;
    out[y][x] = (r.oneIn(2)?'>':'<');
    break;
  }
}

// Cranes: shore birds that patrol edges of shallow water.
if (cranes < CRANE_CAP && r.oneIn(40)) {
  for (int tries=0; tries<5000; ++tries) {
    ...
    for (int dy=-1; dy<=1 && !shore; ++dy) for (int dx=-1; dx<=1 && !shore; ++dx) {
      if (!dx && !dy) continue;
      if (w.water[y+dy][x+dx] > 0 && w.water[y+dy][x+dx] <= 2) shore=true;
    }
    if (!shore) continue;
    out[y][x] = 'C';
    break;
...
for (int y=0; y<H; ++y) for (int x=0; x<W; ++x) {
    ...
    if (e=='>' || e=='<') {
  // Fish drift in water; they like kelp cover and deeper spots.
  if (w.water[y][x] == 0) { out[y][x]=' '; continue; }
  int bestNx=x, bestNy=y, bestScore=-999999;
  for (int dy=-1; dy<=1; ++dy) for (int dx=-1; dx<=1; ++dx) {
    if (!dx && !dy) continue;
    int nx=x+dx, ny=y+dy;
    ...
    if (w.water[ny][nx] < 1) continue;
    int score = (int)w.water[ny][nx]*6;
    if (w.terrain[ny][nx]==KELP_GLYPH) score += 18;
    // slight wind/current bias
    score += (dx==w.wind.dx ? 2 : 0) + (dy==w.wind.dy ? 2 : 0);
    score -= (std::abs(dx)+std::abs(dy)==2) ? 1 : 0;
    if (score > bestScore) { bestScore=score; bestNx=nx; bestNy=ny; }
  }
  if (bestNx!=x || bestNy!=y) { out[bestNy][bestNx]=e; out[y][x]=' '; }
  continue;
}
if (e=='C') {
  // Cranes patrol shorelines and occasionally snatch fish.
  // They avoid deep water; stay within 1 tile of water.
  auto isGood = [&](int nx,int ny)->bool{
    if (!inBounds(nx,ny)) return false;
    if (out[ny][nx] != ' ') return false;
    if (w.water[ny][nx] > 0) return false; // crane stays on land
    bool near=false;
    for (int dy=-1; dy<=1 && !near; ++dy) for (int dx=-1; dx<=1 && !near; ++dx) {
      if (!dx && !dy) continue;
      int ax=nx+dx, ay=ny+dy;
      if (!inBounds(ax,ay)) continue;
      if (w.water[ay][ax] > 0 && w.water[ay][ax] <= 2) near=true;
    }
    return near;
  };
  // eat adjacent fish (instant)
  for (int dy=-1; dy<=1; ++dy) for (int dx=-1; dx<=1; ++dx) {
    if (!dx && !dy) continue;
    int ax=x+dx, ay=y+dy;
    if (!inBounds(ax,ay)) continue;
    if (cur[ay][ax]=='>' || cur[ay][ax]=='<') { out[ay][ax]=' '; }
  }
  int nx=x, ny=y;
  int best=-9999;
  for (int dy=-1; dy<=1; ++dy) for (int dx=-1; dx<=1; ++dx) {
    if (!dx && !dy) continue;
    int tx=x+dx, ty=y+dy;
    if (!isGood(tx,ty)) continue;
    int score = 0;
    // prefer tiles adjacent to fish (hunting)
    for (int qy=-1;qy<=1;++qy) for (int qx=-1;qx<=1;++qx) {
      int bx=tx+qx, by=ty+qy;
      if (!inBounds(bx,by)) continue;
      if (cur[by][bx]=='>' || cur[by][bx]=='<') score += 5;
    }
    // mild randomness
    score += r.i(0,2);
    if (score>best){best=score; nx=tx; ny=ty;}
  }
  if (nx!=x || ny!=y) { out[ny][nx]=e; out[y][x]=' '; }
  continue;
}
...
if (d > 0) {
  ...
  int foamChance = shore ? (90 - 10*std::clamp(w.wind.strength,0,5)) : 220;
...
static RGB fgForChar(const World& w, char c, Season s, float sp, int tick, int x, int y) {
  ...
  // Water depth glyphs from waterGlyph(): ASCII-safe digits '1'..'7'
    ...
    if (c == FOAM_GLYPH) { r += 80; g += 80; b += 70; }
...
enum ScaleType { SCALE_CHROMATIC=0, SCALE_MAJOR=1, SCALE_MINOR=2, SCALE_PENTATONIC=3, SCALE_DORIAN=4 };
...
struct SynthOut {
  ...
  // Choose a FluidSynth audio driver. If empty, we will try a few common ones.
  ...
  static void applyDeviceSetting(fluid_settings_t* s, const std::string& driver, const std::string& device) {
    if (!s || device.empty()) return;
    // FluidSynth uses per-driver device keys.
    if (driver=="alsa")       fluid_settings_setstr(s, "audio.alsa.device", device.c_str());
    else if (driver=="pulseaudio") fluid_settings_setstr(s, "audio.pulseaudio.device", device.c_str());
    else if (driver=="pipewire")   fluid_settings_setstr(s, "audio.pipewire.device", device.c_str());
    else if (driver=="jack")       fluid_settings_setstr(s, "audio.jack.id", device.c_str());
  }

  bool open(const std::string& sf2Path, float g, const std::string& wantDriver="", const std::string& wantDevice="") {
    close();
    gain = g;
    audioDriver = wantDriver;
    audioDevice = wantDevice;
    ...
    if (!synth) { close(); return false; }

    sfid = fluid_synth_sfload(synth, sf2Path.c_str(), 1);
    if (sfid < 0) { close(); return false; }

    // Program 0 on ch0 for melodic; ch9 drums.
    fluid_synth_program_change(synth, 0, 0);
    fluid_synth_program_change(synth, 9, 0);

    auto tryDriver = [&](const std::string& driver)->bool{
      fluid_settings_setstr(settings, "audio.driver", driver.c_str());
      applyDeviceSetting(settings, driver, audioDevice);
    ...
    if (!audioDriver.empty()) {
      if (!tryDriver(audioDriver)) { close(); return false; }
    } else {
      // Try common modern Linux stacks first.
      if (!tryDriver("pipewire")) {
        close(); // reset before next attempt
        settings = new_fluid_settings();
        if (!settings) return false;
        fluid_settings_setnum(settings, "synth.gain", (double)gain);
        fluid_settings_setnum(settings, "synth.sample-rate", (double)sampleRate);
        ...
        if (!tryDriver("pulseaudio")) {
          ...
          if (!tryDriver("alsa")) { close(); return false; }
        }
      }
    }
  ...
  void noteOn(int ch, int key, int vel)  { if (enabled && synth) fluid_synth_noteon(synth, ch, key, vel); }
  void noteOff(int ch, int key)         { if (enabled && synth) fluid_synth_noteoff(synth, ch, key); }
  void cc(int ch, int ccnum, int val)   { if (enabled && synth) fluid_synth_cc(synth, ch, ccnum, val); }
#else
  ...
  void cc(int,int,int) {}
    ...
    want.channels = 2;
    want.samples = 1024;
    want.callback = &SynthOut::audioCb;
    want.userdata = this;
...
static void render(SDL_Renderer* ren, const Layout& L, World& w, GlyphCache& gc, int tick, bool showMenu, int menuPage, const std::vector<MidiParam>& params, int menuSel, bool midiEnabled, bool midiClockOut, bool useSimClock, int rootKey, ScaleType scaleType, bool synthEnabled, const std::string& sf2Path) {
  Season s = seasonAt(tick);
...
if (showMenu) {
  ...
  auto pageName = [&](int p)->const char*{
    switch(p&3){ case 0: return "MIDI"; case 1: return "WATER"; case 2: return "SPAWNS"; case 3: return "AUDIO"; }
    return "?";
  };
  ...
  snprintf(buf, sizeof(buf),
           ...
           synthEnabled ? "ON" : "OFF");
  drawString(ren, gc, tx, ty, buf, 240,240,240, 230, L.scale);
  ty += 10*L.scale;

  if ((menuPage&3)==0) {
    drawString(ren, gc, tx, ty, "MIDI/CC (UP/DOWN select, +/- weight, O MIDI, C clock, V clock src, K key, S scale, M menu)", 180,180,180, 220, L.scale);
    ty += 10*L.scale;
    ...
    for (int i=0; i<(int)params.size(); ++i) {
      ...
      snprintf(buf, sizeof(buf), "%s  CC:%d  w:%.2f  v:%.2f", p.name.c_str(), p.cc, p.weight, p.value01);
      drawString(ren, gc, tx, ty + i*9*L.scale, buf, (uint8_t)rr,(uint8_t)gg,(uint8_t)bb, 230, L.scale);
    ...
    drawString(ren, gc, tx, ty, buf, 200,220,240, 230, L.scale);
    ty += 10*L.scale;
    drawString(ren, gc, tx, ty, "Tip: wind now drives lily pad drift + increases foam on shores/crests.", 180,190,200, 220, L.scale);
    ...
    drawString(ren, gc, tx, ty, buf, 220,220,220, 230, L.scale);
    ty += 10*L.scale;
    drawString(ren, gc, tx, ty, "Legendary couple: glyphs 0x19 and 0x1A (ultra rare, never despawn).", 180,180,180, 220, L.scale);
  } else {
    snprintf(buf, sizeof(buf), "SoundFont: %s", sf2Path.empty() ? "(none)" : sf2Path.c_str());
    drawString(ren, gc, tx, ty, buf, 220,220,220, 230, L.scale);
    ty += 10*L.scale;
    drawString(ren, gc, tx, ty, "Use: --synth --sf2 <path.sf2> --gain <0..2>  (compile with -DUSE_FLUIDSYNTH)", 180,180,180, 220, L.scale);
  }
}
...
int main(int argc, char** argv) {
  ...
  for (int i=1; i<argc; ++i) {
    ...
    // Optional: choose FluidSynth audio backend/device on Linux
  ...
  // If we're using FluidSynth's own audio driver, we don't need SDL audio and avoiding it can prevent ALSA lockups.
  Uint32 sdlFlags = SDL_INIT_VIDEO;
#if !defined(USE_FLUIDSYNTH)
  sdlFlags |= SDL_INIT_AUDIO;
#else
  if (!wantSynth) sdlFlags |= SDL_INIT_AUDIO;
#endif
  ...
  if (wantSynth) {
    if (sf2Path.empty()) {
      ...
      if (!synth.open(sf2Path, std::clamp(synthGain, 0.0f, 2.0f), synthAudioDriver, synthAudioDevice)) {
        std::cerr << "[synth] Failed to start synth. Try: --audio-driver pipewire (or pulseaudio/alsa) and check --sf2 path.\n";
      } else {
#ifdef USE_FLUIDSYNTH
        std::cerr << "[synth] OK (driver=" << synth.audioDriver << ")\n";
#endif
      }
    }
  }
...
case SDLK_TAB: if (showMenu) menuPage = (menuPage + 1) & 3; break;
          ...
          case SDLK_PERIOD:
            if (paused) { step(world, r, banner, tick); tick++; // Simple emergent music: map world state -> notes (built-in synth only).
if (synth.enabled && (tick % 24)==0) {
  // release previous
  if (heldNote>=0) synth.noteOff(0, heldNote, 0);
  if (heldNote2>=0) synth.noteOff(0, heldNote2, 0);

  int waterTiles=0, plants=0, predators=0;
  for (int yy=0; yy<H; ++yy) for (int xx=0; xx<W; ++xx) {
  ...
  float p01 = (float)plants / (float)(W*H);

  int base = 48 + (int)(w01*18.0f); // C3..ish upward with water
  int n1 = quantizeNoteToScale(base + (int)(p01*12.0f), rootKey, scaleType);
  int n2 = quantizeNoteToScale(base + 7 + (predators%5), rootKey, scaleType);

  heldNote = std::clamp(n1, 36, 84);
  heldNote2 = std::clamp(n2, 36, 84);
  synth.noteOn(0, heldNote, 92);
  if (!r.oneIn(3)) synth.noteOn(0, heldNote2, 64);

  // percussion on storms / lightning-ish
  if (world.weather.state==STORM && r.oneIn(2)) synth.noteOn(9, 36, 96); // kick
  if (world.weather.state==RAIN && r.oneIn(2)) synth.noteOn(9, 42, 70);  // closed hat
}
}
            break;
...
render(ren, layout, world, gc, tick, showMenu, menuPage, params, menuSel, midi.enabled, midiClockOut, useSimClock, rootKey, scaleType, synth.enabled, sf2Path);
    SDL_Delay(6);
  }
