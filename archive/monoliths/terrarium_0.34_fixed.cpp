...
static void updateWeather(World& w, Rng& r, int tick) {
  ...
  switch (w.weather.state) {
    case CLEAR: {
      ...
      if (world.biome==TROPICAL) chance = std::max(120, chance-60);
    ...
    case RAIN: {
      ...
      if (world.biome==TROPICAL) stormChance = std::max(120, stormChance-80);
  ...
  if (w.weather.lastTickWasRaining && !isRainingNow) {
    ...
    if (world.biome==TROPICAL) chance = std::max(1, chance-1);
...
static void seedWorld(World& w, Rng& r, Biome biome) {
  ...
  w.cloudOpacity = 1.0f;
  if (biome==TROPICAL) w.cloudOpacity = 0.55f;
  if (biome==DESERT)   w.cloudOpacity = 0.45f;
  ...
  if (biome == DESERT) {
    for (int k=0; k< (W*H)/180; ++k) {
      int x=r.i(0,W-1), y=r.i(0,H-1);
      if (w.water[y][x]>0) continue;
      if (w.terrain[y][x]=='`' && r.oneIn(3)) w.terrain[y][x]='c';
    }
  }
  ...
  for (int k=0; k< (W*H)/2200; ++k) {
    ...
    if (w.water[y][x]==0 && (w.terrain[y][x]=='#' || w.terrain[y][x]==',') && r.u01() < 0.35f*w.bw.treeChance)
      w.terrain[y][x] = (world.biome==TROPICAL ? (r.oneIn(2)?'P':(r.oneIn(2)?'T':'Y')) : (r.oneIn(2) ? 'T' : 'Y'));
  }
...
static void waterSinks(World& w, Rng& r, Season s) {
  ...
  float infil = 0.0010f;
  if (world.biome == TROPICAL) infil *= 0.85f;
  if (s == SUMMER) infil *= 1.15f;
  if (s == WINTER) infil *= 0.75f;

  float edgeDrain = 0.0040f;
  if (world.biome == WETLAND) edgeDrain *= 0.60f;
  if (world.biome == TROPICAL) edgeDrain *= 0.70f;
...
static void stepTerrain(World& w, Rng& r, Season s, int tick) {
  ...
  for (int y=0; y<H; ++y) for (int x=0; x<W; ++x) {
    ...
    if (isVeg(c)) {
      ...
      for (int dy=-1; dy<=1 && !ignite; ++dy) for (int dx=-1; dx<=1 && !ignite; ++dx) {
        ...
        if (w.terrain[ny][nx]=='*') {
          ...
          if (world.biome==TROPICAL && w.weather.state==STORM) denom = std::max(2, denom-1);
    ...
    if (c=='.') {
      ...
      if (world.biome==TROPICAL) p *= 1.25f;
    ...
    if (c=='#') {
      if ((sh+tr)>=3 && wet>0 && r.oneIn((int)(280*winterSlow)))
        next[y][x]=(world.biome==TROPICAL ? (r.oneIn(2)?'P':(r.oneIn(2)?'T':'Y')) : (r.oneIn(2)?'T':'Y'));
      if (wet==0 && r.oneIn((int)(170*winterSlow))) next[y][x]='"';
      if (wet>0 && r.u01() < 0.006f * autumnMush * rainBoost * w.bw.mushChance) next[y][x]='m';
      if (s==SUMMER && r.oneIn(220) && world.biome!=ALPINE) next[y][x]='$';
      continue;
    }
    ...
    if (c=='f' || c=='+' || c=='&' || c=='!' ) {
      ...
      if (world.biome==TROPICAL) fade = (int)(fade * 1.25f);
...
static void maybeChaos(World& w, Rng& r, std::string& banner, Season s) {
  int base = (world.biome==ALIEN) ? 900 : (world.biome==TROPICAL ? 1100 : 1400);
...
static void stepEntities(World& w, Rng& r, Season s, int tick) {
  ...
  if (world.biome != ALPINE && rabbits < RABBIT_CAP_BASE && r.oneIn(10)) {
  ...
  if ((world.biome==MEADOW || world.biome==WETLAND || world.biome==TROPICAL) && deer < DEER_CAP_BASE && r.oneIn(140)) {
...
static const uint8_t* glyph8(char c) {
  ...
  static const uint8_t LIZ[8]    = {0x00,0x3C,0x66,0x3C,0x18,0x3C,0x66,0x00}; // L
  static const uint8_t CAM[8]    = {0x00,0x66,0x7E,0x5A,0x7E,0x66,0x3C,0x00}; // C
  static const uint8_t DOLP[8]   = {0x00,0x3C,0x7E,0x66,0x7E,0x3C,0x18,0x00}; // D
  static const uint8_t WHAL[8]   = {0x00,0x7E,0xDB,0xFF,0xFF,0xDB,0x7E,0x00}; // W
  static const uint8_t SEAM[8]   = {0x18,0x3C,0x7E,0xFF,0x7E,0x3C,0x5A,0x00}; // S
...
int main(int argc, char** argv) {
  ...
  while (running) {
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.34 | biome ") + biomeName(biome) +
