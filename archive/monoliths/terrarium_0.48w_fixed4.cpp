...
struct Rng {
  ...
  float u01(){ std::uniform_std::uniform_real_distribution<float> d(0.f,1.f); return d(rng); }
...
struct MidiParam { const char* name; int cc; float weight; float value01; };
...
if (showMenu) {
  ...
  snprintf(buf, sizeof(buf), "Biome: %s   Season: %s   MIDI:%s  ClockOut:%s  Clock:%s  Root:%d  Scale:%d",
           biomeName(w.biome), seasonName(s),
