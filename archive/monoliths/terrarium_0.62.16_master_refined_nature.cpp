...
static void stepEntities(World& w, Rng& r, Season s, int tick) {
...

  {
    ...
    for (auto &a : w.agents) {
      ...
      // Inertia: prefer to keep heading when possible.
      if (nx==a.x && ny==a.y && (a.lastDx!=0 || a.lastDy!=0) && r.u01() < 0.60f) {
        int tx = a.x + a.lastDx;
        int ty = a.y + a.lastDy;
        if (inBounds(tx,ty)) {
          if (isAquaticTraits(a.traits)) {
            if (w.water[ty][tx] > 0) { nx=tx; ny=ty; }
          } else {
            if (w.water[ty][tx] == 0) { nx=tx; ny=ty; }
          }
        }
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.62.17 | biome ") + biomeName(biome) +
