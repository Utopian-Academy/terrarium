...
static void stepEntities(World& w, Rng& r, Season s, int tick) {
...

  {
    ...
    for (auto &a : w.agents) {
      ...
      // Movement: driven by needs, with inertia + slow drift.
      int ox=a.x, oy=a.y;
      int nx=a.x, ny=a.y;
      ...
      // Smooth velocity toward desired direction (sub-tile motion).
      int desiredDx = nx - a.x;
      int desiredDy = ny - a.y;
      float dlen = std::sqrt((float)(desiredDx*desiredDx + desiredDy*desiredDy));
      float dirx = (dlen > 0.01f) ? (float)desiredDx / dlen : 0.f;
      float diry = (dlen > 0.01f) ? (float)desiredDy / dlen : 0.f;

      float baseSpeed = 0.22f + 0.20f * fd.speed;
      if (night && !isPredatorTraits(a.traits)) baseSpeed *= 0.6f;
      if (a.intent==INTENT_FLEE || a.intent==INTENT_HUNT) baseSpeed *= 1.25f;
      float desiredVx = dirx * baseSpeed;
      float desiredVy = diry * baseSpeed;
      ...
      if (isAquaticTraits(a.traits)) {
        if (w.water[ty][tx]==0) { a.vx *= 0.2f; a.vy *= 0.2f; a.px = (float)a.x; a.py = (float)a.y; }
      } else {
        if (w.water[ty][tx]>0) { a.vx *= 0.2f; a.vy *= 0.2f; a.px = (float)a.x; a.py = (float)a.y; }
      }
