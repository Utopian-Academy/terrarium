...
if (chordOn) {
  ...
  int idx = 0;
  if (arp01 < 0.25f) {
    idx = arpStep % toneCount; // up
  } else if (arp01 < 0.50f) {
    idx = (toneCount - 1) - (arpStep % toneCount); // down
  } else if (arp01 < 0.75f) {
    int span = toneCount*2 - 2;
    int p = (span > 0) ? (arpStep % span) : 0;
    idx = (p < toneCount) ? p : (span - p); // up-down
  } else {
    idx = r.irange(0, toneCount-1); // random
  }
  arpStep++;
  ...
  int note1 = baseRoot + tones[idx];
  int note2 = baseRoot + tones[(idx + 2) % toneCount];
