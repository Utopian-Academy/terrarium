...
static void agentsWriteToGrid(World& w, int tick){
  ...
  for(int y=0;y<H;++y) for(int x=0;x<W;++x) w.fauna[y][x]=' ';
  for(auto &a: w.agents){
...
static void stepEntities(World& w, Rng& r, Season s, int tick) {
...

  {
    ...
    for (auto &a : w.agents) {
      ...
      // Turn-rate + inertia: keep heading unless strong intent.
      {
        int desiredDx = nx - a.x;
        int desiredDy = ny - a.y;
        float turnRate = 0.35f;
        if (a.intent==INTENT_FLEE || a.intent==INTENT_HUNT || a.intent==INTENT_DRINK || a.intent==INTENT_FORAGE) {
          turnRate = 0.80f;
        }
        if (a.lastDx!=0 || a.lastDy!=0) {
          if (desiredDx==0 && desiredDy==0) {
            if (r.u01() < 0.35f) { desiredDx = a.lastDx; desiredDy = a.lastDy; }
          } else if (desiredDx!=a.lastDx || desiredDy!=a.lastDy) {
            if (r.u01() > turnRate) { desiredDx = a.lastDx; desiredDy = a.lastDy; }
          }
        }
        nx = a.x + desiredDx;
        ny = a.y + desiredDy;
      ...
      // Final clamp: max one tile per update (diagonals allowed).
      int dx1 = std::clamp(nx - a.x, -1, 1);
      int dy1 = std::clamp(ny - a.y, -1, 1);
      nx = a.x + dx1;
      ny = a.y + dy1;
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.62.19 | biome ") + biomeName(biome) +
