...
enum Biome { MEADOW=0, WETLAND=1, ALPINE=2, ALIEN=3, TROPICAL,
  DESERT=4 };
...
static void seedWorld(World& w, Rng& r, Biome biome) {
  ...
  for (int p=0; p<ponds; ++p) {
    ...
    int rad = (int)(r.i(5, 14) * cloudScale); if (rad < 6) rad = 6;
...
static RGB fgForChar(const World& w, char c, Season s, float sp, int tick, int x, int y) {
  ...
  else if (c=='s') fg = {230, 210, 150}; // sand
      ...
      if (e.type == SDL_KEYDOWN) {
        switch (e.key.keysym.sym) {
          ...
          case SDLK_b: {
            ...
            w.targetBiome = (Biome)(((int)w.biome + 1) % BIOME_COUNT);
            w.biomeFadeDir = +1;
          } break;
