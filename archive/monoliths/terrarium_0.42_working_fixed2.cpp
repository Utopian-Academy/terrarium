...
static void stepEntities(World& w, Rng& r, Season s, int tick) {
  ...
  int scorpions = countEnt((char)0x01);
  int dragonflies = countEnt((char)0x02);
  int crabs = countEnt((char)0x03);
  int jellies = countEnt((char)0x04);
  int crawlers = countEnt((char)0x05);
  int orbs = countEnt((char)0x06);
  ...
  if (w.biome==DESERT && scorpions < SCORPION_CAP && r.oneIn(22)) {
    for (int tries=0; tries<2000; ++tries) {
      int x=r.i(0,W-1), y=r.i(0,H-1);
      if (!canSpawnAt(x,y)) continue;
      if (w.terrain[y][x]=='s' || w.terrain[y][x]=='.') { out[y][x]=(char)0x01; break; }
    }
  }
  ...
  if (w.biome==TROPICAL && jellies < JELLY_CAP && r.oneIn(26)) {
    for (int tries=0; tries<2600; ++tries) {
      int x=r.i(0,W-1), y=r.i(0,H-1);
      if (out[y][x] != ' ') continue;
      if (w.water[y][x] >= 4) { out[y][x]=(char)0x04; break; }
    }
  }
...
static const uint8_t HER1 [8] = {0x00,0x18,0x18,0x3C,0x7E,0x18,0x3C,0x00}; // \x1A
  ...
  switch (c) {
    ...
    case '\x01': return SCOR1;
    case '\x02': return DRGN1;
    case '\x03': return CRAB1;
    case '\x04': return JELY1;
    case '\x05': return CRAW1;
    case '\x06': return ORB1;
