...
// while still allowing an explicit override via MIDI param "Instr" (0..1).
  ...
  if (!instrExplicit && tick >= nextAutoChangeTick) {
    ...
    float pick = std::clamp(t + (r.frand()*0.40f - 0.20f), 0.0f, 0.9999f);
...
if (world.season == WINTER) rootAuto = (rootAuto + 10) % 12; // -2
else if (world.season == SPRING) rootAuto = (rootAuto + 0) % 12;
else if (world.season == SUMMER) rootAuto = (rootAuto + 2) % 12;
else if (world.season == FALL)   rootAuto = (rootAuto + 0) % 12;
...
int jitter = (int)std::lround((r.frand()*2.f-1.f) * (1.f + 5.f*fauna01 + 2.f*wind01));
