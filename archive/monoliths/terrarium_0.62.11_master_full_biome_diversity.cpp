...
static constexpr int MAX_FLORA  = 2000;
...
static void updateWeather(World& w, Rng& r, int tick) {
  ...
  float seasonRain = (s==SPRING?1.15f : s==SUMMER?0.90f : s==AUTUMN?1.00f : 0.65f);
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.62.12 | biome ") + biomeName(biome) +
