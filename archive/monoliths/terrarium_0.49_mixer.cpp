...
static float g_voiceFader[NUM_VOICES] = {1.f, 1.f, 1.f}; // 0..1
...
static inline void applyVoiceMixer(SynthOut& synth){
  ...
  for (int v=0; v<NUM_VOICES; ++v){
    ...
    synth.cc(ch, 7,  cc127f(f)); // Channel Volume
...
if (showMenu) {
  ...
  } else if ((menuPage%6)==4) { // page 4: VOICES
...
case SDLK_UP: {
            int maxSel = 0;
            if (showMenu) {
              if ((menuPage%6)==0) maxSel = (int)params.size();
              else if ((menuPage%6)==1) maxSel = 7;
              else if ((menuPage%6)==4) maxSel = NUM_VOICES*3;
              else if ((menuPage%6)==5) maxSel = NUM_VOICES+1;
              else maxSel = 1;
