...
// ===== Config =====
static constexpr const char* VERSION = "0.70.03";
...
struct SynthOut {
  ...
  bool open(const std::string& sf2, float g, const std::string& driver, const std::string& device){
    ...
    // Period size matches PipeWire's default quantum (1024 @ 48kHz = ~21ms).
    // Using 3 periods = ~64ms total headroom — tight but glitch-free with RT priority.
