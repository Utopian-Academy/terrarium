...
render(ren, layout, world, gcWorld, gcText, tick, showMenu, menuPage, params, menuSel, midi.enabled, midiClockOut, useSimClock, rootKey, scaleType, synth.enabled, sf2Path);
    SDL_Delay(6);
  }
...
SDL_Quit();
  ...
  for(char c: s){
    ...
    SDL_Texture* tex = gcWorld.get(ren, (unsigned char)c);
