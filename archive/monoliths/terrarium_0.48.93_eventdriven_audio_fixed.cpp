...
// (removed duplicate SimAudioEvents/gAudioEv definition)
...
const int totalEv = gAudioEv.floraGrows + gAudioEv.floraDies + gAudioEv.attacks + gAudioEv.scares + gAudioEv.agentDeaths;
if (totalEv == 0) return;
...
if ((gAudioEv.floraDies + gAudioEv.agentDeaths) > 0 && (tick - lastDeathTick) >= 3) {
  int amt = gAudioEv.floraDies + gAudioEv.agentDeaths;
