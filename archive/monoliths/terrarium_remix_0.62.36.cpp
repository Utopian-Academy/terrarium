...
static void stepWater(World& w, Rng& r){
  ...
  const int threads = std::max(1, g_threads);
  const int simThreads = std::min(threads, H);
  std::vector<WaterF> deltas(simThreads, WaterF(H, std::vector<float>(W, 0.f)));
  ...
  parallelForRange(H, simThreads, [&](int y0, int y1, int tid){
    ...
    for(int y=y0;y<y1;++y){
      for(int x=0;x<W;++x){
        float v = w.water[y][x];
        for (int t=0; t<simThreads; ++t) v += deltas[t][y][x];
...
static void stepAgents(World& w, Rng& r, int tick){
  ...
  std::vector<AgentSense> sense;
  bool useSense = (g_threads > 1) && !w.agents.empty();
  if (useSense) {
    ...
    sense.assign(views.size(), AgentSense{});
    const int threads = std::max(1, g_threads);
...
static void render(SDL_Renderer* ren, const Layout& L, World& w, GlyphCache& gcWorld, GlyphCache& gcText, int tick, bool showMenu, int menuPage, int menuSel){
  ...
  for(int y=0;y<viewH;++y){
    ...
    for(int x=0;x<viewW;++x){
      ...
      if (gt) {
        RGB fg = fgForChar(w, c, wx, wy, seasonAt(tick));
  ...
  // on-screen legend
  if (!showMenu) {
    drawString(ren, gcText, 12, 10, "M menu  B biome  R reseed  [ ] speed  WASD pan  Z/X zoom  G ghibli  F5 preset  F6 save  F7 midi  F9 audio  F11 fullscreen", 190,190,200, 230, 1);
  }
