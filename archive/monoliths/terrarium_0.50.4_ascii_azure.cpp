...
static constexpr int START_AGENTS = 4;
static bool g_asciiOnly = false;
...
static inline char asciiRemap(char c){
  unsigned char u = (unsigned char)c;
  if (u < 32) return '~';
  if (u >= 0x80u) return '?';
  return c;
}
static inline char asciiWaterGlyph(const World& w, int x, int y, int tick);
...
// Species-based ecology (used for spawn + behavior).
static inline bool isAquaticSpecies(uint8_t s){
  return (s==SPEC_SHELLBACK || s==SPEC_SWARMER || s==SPEC_PARASITE);
}
static inline bool isHerbivoreSpecies(uint8_t s){
  return (s==SPEC_WANDERER || s==SPEC_SHELLBACK || s==SPEC_SWARMER || s==SPEC_ENGINEER || s==SPEC_MYSTIC || s==SPEC_TRICKSTER);
}
static inline bool isPredatorSpecies(uint8_t s){
  return (s==SPEC_PACKHUNTER || s==SPEC_PARASITE);
}
static inline bool isBirdSpecies(uint8_t /*s*/){ return false; }
...
static inline char speciesDisplayGlyph(uint8_t spec, Biome /*b*/, int tick, bool legendA, bool legendB){
  ...
  if (g_asciiOnly) {
    // ASCII-only experimental glyphs (2-frame toggles).
    static const char frames[SPEC_COUNT][2] = {
      {'o','O'}, // wanderer
      {'u','U'}, // shellback
      {':',';'}, // swarmer
      {'e','E'}, // engineer
      {'%','p'}, // parasite (butterfly-ish)
      {'n','N'}, // packhunter
      {'*','+'}, // mystic
      {'?','!'}  // trickster
    };
    return frames[(int)(spec % SPEC_COUNT)][ph];
  }
...
static inline char asciiWaterGlyph(const World& w, int x, int y, int tick){
  ...
  int ph = (tick/6 + x/3 + y/5) & 3;
  uint32_t h = hash3((uint32_t)(x + tick/7), (uint32_t)(y - tick/11), 0xA5A5u);
  bool crest = (w.wind.strength >= 2) && ((h % (uint32_t)(22 - 3*w.wind.strength))==0u);
  if (shore || crest) return '=';
  if (d <= 2) return (ph & 1) ? '~' : '=';
  if (d <= 4) return (ph==0) ? '~' : '?';
  return (ph==0) ? '?' : ((ph==1) ? '?' : '~');
}
...
static void updateModPool(World& w, int tick, int viewW, int viewH){
  ...
  for (auto &a: w.agents){
    if (a.x<x0 || a.x>=x1 || a.y<y0 || a.y>=y1) continue;
    if (isPredatorSpecies(a.species)) preds++; else prey++;
  }
...
static void agentsInitFromGrid(World& w, Rng& r){
  ...
  for(int y=0;y<H;++y){
    for(int x=0;x<W;++x){
      ...
      if(g=='.' || g==' ') continue;
      Agent a; a.id=nextId++; a.x=x; a.y=y; a.species = pickBiomeSpecies(w.biome, r);
      a.glyph = speciesDisplayGlyph(a.species, w.biome, 0, false, false);
...
static inline bool edgeCellForHabitat(const World& w, int x, int y, bool aquatic){
  if (!inBounds(x,y)) return false;
  if (aquatic) return (w.water[y][x] > 0);
  if (w.water[y][x] > 0) return false;
  return cellPassableForAgent(w, x, y);
}

static inline void pickEdgeCell(Rng& r, int &x, int &y, int band=2){
  int side = r.irange(0,3);
  int bx = r.irange(0, band);
  int by = r.irange(0, band);
  if (side==0) { x = bx; y = r.irange(0, H-1); }
  else if (side==1) { x = W-1-bx; y = r.irange(0, H-1); }
  else if (side==2) { y = by; x = r.irange(0, W-1); }
  else { y = H-1-by; x = r.irange(0, W-1); }
}

static bool spawnOneAgent(World& w, Rng& r, uint8_t spec, bool edgeOnly){
  if ((int)w.agents.size() >= MAX_AGENTS) return false;
  bool aquatic = isAquaticSpecies(spec);
  for (int tries=0; tries<240; ++tries){
    int x=0,y=0;
    if (edgeOnly) pickEdgeCell(r, x, y, 2);
    else { x=r.i(0,W-1); y=r.i(0,H-1); }
    if (!edgeCellForHabitat(w, x, y, aquatic)) continue;
    bool occ=false;
...
static void spawnInitialAgents(World& w, Rng& r){
  int attempts = 0;
  while ((int)w.agents.size() < START_AGENTS && attempts < START_AGENTS*6) {
    bool wantAquatic = r.oneIn(2);
    uint8_t spec = pickBiomeSpecies(w.biome, r);
    if (wantAquatic && !isAquaticSpecies(spec)) spec = SPEC_SHELLBACK;
    if (!wantAquatic && isAquaticSpecies(spec)) spec = SPEC_WANDERER;
    (void)spawnOneAgent(w, r, spec, false);
    attempts++;
  }
}
...
static inline int nearestPredatorDist(const World& w,int x,int y,int radius){
  int best=9999;
  for(auto &p: w.agents){
    if(!isPredatorSpecies(p.species)) continue;
...
static void stepEntities(World& w, Rng& r, Season s, int tick) {
  ...
  if (doUpdate) {
    for (auto &a : w.agents) {
      ...
      if (!isPredatorSpecies(a.species) && pd <= 6) {
      ...
      if (isHerbivoreSpecies(a.species) && isEdiblePlant(tile)) {
      ...
      if (isPredatorSpecies(a.species)) {
        for (auto &prey : w.agents) {
          if (&prey==&a) continue;
          if (isPredatorSpecies(prey.species)) continue;
...
// Movement: very simple, but driven by needs and panic.
      ...
      } else if (a.hunger > 0.65f && isHerbivoreSpecies(a.species)) {
      ...
      if (a.x!=ox || a.y!=oy) {
        StepEvent ev; ev.x=a.x; ev.y=a.y; ev.dx=a.x-ox; ev.dy=a.y-oy; ev.species=a.species;
        ev.strength = (std::abs(ev.dx)+std::abs(ev.dy) > 1) ? 2.f : 1.f;
        g_stepEvents.push_back(ev);
      }
    }

    // Rare immigration at edges, capped.
    if ((int)w.agents.size() < MAX_AGENTS) {
      float denomF = 240.0f / std::max(0.15f, g_alea.spawnChance);
      int denom = (int)std::max(120.0f, denomF);
      if (r.oneIn(denom)) {
        uint8_t spec = pickBiomeSpecies(w.biome, r);
        (void)spawnOneAgent(w, r, spec, true);
      }
    }

    // Rare emigration: non-legendary agents may leave via edges.
    w.agents.erase(std::remove_if(w.agents.begin(), w.agents.end(),
      [&](const Agent& a){
        if (isLegendary(a)) return false;
        bool edge = (a.x<=0 || a.y<=0 || a.x>=W-1 || a.y>=H-1);
        return edge && r.oneIn(600);
      }), w.agents.end());
...
static Layout computeLayout(SDL_Renderer* ren) {
  ...
  L.hudH = g_showHud ? std::max(40, L.screenH/18) : 0;
  L.simHpx = L.screenH; // sim always fills screen; HUD overlays
  return L;
}
...
static RGB fgForChar(const World& w, char c, Season s, float sp, int tick, int x, int y) {
  ...
  // ASCII water (azure palette)
  else if (g_asciiOnly && inBounds(x,y) && w.water[y][x] > 0) {
    uint8_t d = std::min<int>(7, (int)w.water[y][x]);
    bool shore=false;
    for (int dy=-1; dy<=1 && !shore; ++dy) for (int dx=-1; dx<=1 && !shore; ++dx) {
      if (!dx && !dy) continue;
      int nx=x+dx, ny=y+dy;
    ...
    int r = 18 + d*2 + shimmer;
    int g = 120 + d*8 + shimmer*2;
    int b = 210 + d*9 + shimmer*3;
    if (shore || c=='=') { r = 190; g = 235; b = 255; }
  ...
  for (const auto& rp : g_ripples) {
    ...
    // Push fauna/agents on the shock-ring (actual simulation chaos, not just visual)
    {
      ...
      for (auto &a : w.agents) {
        ...
        if (!isAquaticSpecies(a.species) && w.water[nyCell][nxCell] > 5) {
          // try sideways jitter
...
if (!g_stepEvents.empty()) {
  ...
  for (int i=0; i<(int)g_stepEvents.size() && used < maxEv; ++i) {
    ...
    int family = 0;
    if (isPredatorSpecies(ev.species)) family = 1;
    else if (isAquaticSpecies(ev.species) || isBirdSpecies(ev.species)) family = 2;
...
for (int y=0; y<viewH; ++y) {
    ...
    for (int x=0; x<viewW; ++x) {
      ...
      if (g_asciiOnly && w.water[wy][wx] > 0 && w.entities[wy][wx]==' ' && w.overlay[wy][wx]==' ') {
        c = asciiWaterGlyph(w, wx, wy, tick);
      }
      if (g_asciiOnly) c = asciiRemap(c);
      ...
      if (gt) {
        ...
        // Azure glow for ASCII water.
        if (g_asciiOnly && w.water[wy][wx] > 0) {
          SDL_SetRenderDrawBlendMode(ren, SDL_BLENDMODE_ADD);
          uint8_t gr = (uint8_t)std::min(255, (int)fg.r + 30);
          uint8_t gg = (uint8_t)std::min(255, (int)fg.g + 40);
          uint8_t gb = (uint8_t)std::min(255, (int)fg.b + 50);
          setColor(ren, gr, gg, gb, 30);
          SDL_RenderFillRect(ren, &rc);
...
for (int i=1; i<argc; ++i) {
  ...
  else if (std::strcmp(argv[i], "--ascii")==0) { g_asciiOnly = true; }
  else if (std::strcmp(argv[i], "--hud")==0 && i+1<argc) {
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.50.4 | biome ") + biomeName(biome) +
