...
static constexpr int MAX_AGENTS = 500;
...
static void spawnInitialAgents(World& w, Rng& r){
  ...
  float dens = 1.0f;
  if (w.biome==DESERT) dens = 0.45f;
  else if (w.biome==ALPINE) dens = 0.60f;
  else if (w.biome==WETLAND) dens = 1.20f;
  else if (w.biome==TROPICAL) dens = 1.35f;
  else if (w.biome==ALIEN) dens = 0.95f;

  // Start modest; let reproduction handle growth.
  const int START_AGENTS_BASE = 28;
  int target = (int)std::lround(START_AGENTS_BASE * dens);
  target = std::clamp(target, 12, 60);

  for (int i=0; i<target; ++i){
    for (int tries=0; tries<200; ++tries){
      int x=r.i(0,W-1), y=r.i(0,H-1);
      uint8_t spec = pickBiomeSpecies(w.biome, r);
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.62.3 | biome ") + biomeName(biome) +
