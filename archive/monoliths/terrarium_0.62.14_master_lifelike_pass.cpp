...
// Movement: very simple, but driven by needs and panic.
      ...
      if (a.flags & 1) {
      ...
      } else if (isBirdTraits(a.traits) && r.u01() < 0.45f) {
      ...
      } else if (isAquaticTraits(a.traits) && r.u01() < 0.45f) {
        a.intent = INTENT_WANDER;
        ...
        if (r.u01() < moveBias) moveRandom(r,nx,ny);
      }
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.62.15 | biome ") + biomeName(biome) +
