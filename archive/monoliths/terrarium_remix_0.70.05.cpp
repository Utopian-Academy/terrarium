...
static std::string g_audioDriver = "pipewire"; // native PipeWire driver: no resampling, proper callback timing; override with --audio-driver
...
// FluidSynth built-in synth (optional)
  ...
  // PipeWire must be initialized before FluidSynth's pipewire audio driver starts.
  // pw_init() sets up SPA plugin loading; without it FluidSynth fails with
  // "can't make support.system handle".
...
struct SynthOut {
  ...
  bool open(const std::string& sf2, float g, const std::string& driver, const std::string& device){
    ...
    // PipeWire native driver: period-size matches PipeWire quantum (1024 @ 48kHz = ~21ms).
    // PipeWire manages its own scheduling so 2 periods is sufficient — it won't underrun
    // the way a polling-based driver (sdl2/pa) would without extra buffering.
...
static inline void resetAudio(SynthOut& synth, MidiOut& midi){
  ...
  g_pendingOffs.clear();
  SDL_Delay(40); // brief drain — ~2 FluidSynth periods at 48kHz/1024
  synth.close();
...
int main(int argc, char** argv){
  // Do NOT init SDL_AUDIO — FluidSynth uses PipeWire natively; SDL audio subsystem
  // would compete for the same PipeWire connection and cause "can't make support.system handle".
...
#ifdef USE_FLUIDSYNTH
  // Must call pw_init() before any PipeWire API (including FluidSynth's pipewire driver).
  // This sets up SPA plugin directory discovery so pw_loop_new() can find support.system.
  ...
  while(running){
    ...
    // Audio watchdog: periodically re-init FluidSynth in case PipeWire dropped the stream.
    // Belt-and-suspenders — PipeWire native driver is more stable than SDL2/PA but can
    // still disconnect if the graph reconfigures. Fires every ~5 min (1800 ticks at ~6 TPS).
