...
if (showMenu) {
  ...
  g_modScroll = std::clamp(g_modScroll, 0, std::max(0, MOD_N-14));
  for(int i=0;i<14;++i){
    int mi = g_modScroll+i;
    char ln[128];
  ...
  for(int i=0;i<MOD_SLOTS;++i){
    ...
    drawString(ren, gcText, tx, ty + i*10*L.scale, ln, (i==g_mmSel)?255:200, (i==g_mmSel)?255:200, 220, 230, L.scale);
    if(i==g_mmSel){
      int caretX = tx + (g_mmField==0?30:(g_mmField==1?92:(g_mmField==2?170:(g_mmField==3?230:0))))*L.scale/2;
  ...
  drawString(ren, gcText, tx, ty + MOD_SLOTS*10*L.scale + 4*L.scale,
      ...
      int sel = clampi(menuSel, 0, nA-1);
      // keep selection synced
      g_inspectIdx = sel;

      const Agent& a = w.agents[sel];
      auto moodName=[&](Mood m)->const char*{
        switch(m){
          case MOOD_CALM: return "calm";
          case MOOD_CURIOUS: return "curious";
          case MOOD_HUNGRY: return "hungry";
          case MOOD_THIRSTY: return "thirsty";
          case MOOD_FEARFUL: return "fearful";
          case MOOD_ENRAGED: return "enraged";
          case MOOD_EUPHORIC: return "euphoric";
        }
        return "?";
      };
      auto intentName=[&](Intent it)->const char*{
        switch(it){
          case INTENT_WANDER: return "wander";
          case INTENT_FORAGE: return "forage";
          case INTENT_DRINK: return "drink";
          case INTENT_FLEE: return "flee";
          case INTENT_HUNT: return "hunt";
          case INTENT_REST: return "rest";
        }
        return "?";
      };

      snprintf(buf,sizeof(buf),"#%d  glyph '%c'  pos %d,%d   species %s", sel, a.glyph, a.x, a.y, speciesName(a.species));
      drawString(ren,gcText,tx,ty,buf, 240,240,240, 230, L.scale); ty += 10*L.scale;

      snprintf(buf,sizeof(buf),"HP: %3d%%   Stress: %3d%%   Hunger: %3d%%   Thirst: %3d%%   Fatigue: %3d%%",
        (int)std::lround(100.f*a.health),
        (int)std::lround(100.f*a.stress),
        (int)std::lround(100.f*a.hunger),
        (int)std::lround(100.f*a.thirst),
        (int)std::lround(100.f*a.fatigue));
      drawString(ren,gcText,tx,ty,buf, 210,210,210, 230, L.scale); ty += 10*L.scale;

      snprintf(buf,sizeof(buf),"Mood: %s   Intent: %s   Panic:%s   Follow:%s",
        moodName(a.mood), intentName(a.intent),
        (a.flags&1)?"yes":"no",
        g_followInspect?"ON":"OFF");
      drawString(ren,gcText,tx,ty,buf, 200,220,200, 230, L.scale); ty += 12*L.scale;

      // Roster preview (top slice)
      int maxRows = std::max(6, (L.hudH/9)-1);
      maxRows = std::min(maxRows, 24);
      int start = 0;
      if (sel >= maxRows) start = sel - maxRows/2;
      ...
      for (int i=0;i<maxRows && (start+i)<nA;i++){
        ...
        snprintf(buf,sizeof(buf),"%4d '%c'  (%d,%d)  %s  hp%3d st%3d hu%3d th%3d",
          idx,q.glyph,q.x,q.y, speciesName(q.species),
          (int)std::lround(100.f*q.health),
          (int)std::lround(100.f*q.stress),
          (int)std::lround(100.f*q.hunger),
          (int)std::lround(100.f*q.thirst));
        drawString(ren,gcText,tx, ty + i*9*L.scale, buf, rr,gg,bb, 230, L.scale);
      }
    }
...
case SDLK_UP:
  if (showMenu && (menuPage%9)==7) { /* MODS scroll */  g_modScroll = std::max(0, g_modScroll-1); }
  else if (showMenu && (menuPage%9)==8) { g_mmSel = std::max(0, g_mmSel-1); }
  break;
case SDLK_DOWN:
  if (showMenu && (menuPage%9)==7) {  g_modScroll = std::min(std::max(0, MOD_N-14), g_modScroll+1); }
  else if (showMenu && (menuPage%9)==8) { g_mmSel = std::min(MOD_SLOTS-1, g_mmSel+1); }
  break;
case SDLK_LEFT:
  if (showMenu && (menuPage%9)==8) { g_mmField = (g_mmField+3)%4; }
  break;
case SDLK_RIGHT:
  if (showMenu && (menuPage%9)==8) { g_mmField = (g_mmField+1)%4; }
  break;
case SDLK_e:
  if (showMenu && (menuPage%9)==8) { static int g_mmSel=0; g_modMap[g_mmSel].enabled = !g_modMap[g_mmSel].enabled; }
  break;
