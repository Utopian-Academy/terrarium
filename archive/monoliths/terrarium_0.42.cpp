...
static inline uint32_t biomeSalt(Biome b) {
  switch (b) {
    ...
    case TROPICAL: return 0x7A0F1u; // will be fixed below
...
static inline uint32_t seasonSalt(Season s) {
  switch (s) {
    ...
    case AUTUMN: return 0xBADC0u;
    case WINTER: return 0x0F00Du;
...
struct ControlBus {
  ...
  void updateFromWorld(const World& w, int tick, int season, float sp) {
    ...
    float windT = std::min(1.f, std::abs(w.wind.vx) + std::abs(w.wind.vy));
...
static void stepSnow(World& w, Rng& r, Season s) {
  ...
  for (int y=0;y<H;++y) for (int x=0;x<W;++x) {
    int i = y*W + x;
    if (waterAt(w,x,y) > 0) { w.snow[i] = 0; continue; }
    uint8_t sn = w.snow[i];
    if (s == WINTER) {
...
static void stepTerrain(World& w, Rng& r, Season s, int tick) {
  ...
  if (w.biome == MEADOW || w.biome == WETLAND || w.biome == TROPICAL) {
    for (int y=1;y<H-1;++y) for (int x=1;x<W-1;++x) {
      ...
      if (nearW && r.oneIn(w.biome==TROPICAL ? 140 : 220)) next[y*W + x] = 'r'; // reeds
      else if (nearW && r.oneIn(520)) next[y*W + x] = 'M'; // moss
      else if (r.oneIn(520)) next[y*W + x] = 'v'; // fern
      else if (r.oneIn(680)) next[y*W + x] = 'c'; // clover
    }
  }
...
// AUTUMN/WINTER seasonal transforms + dieback + density cap
  if (s == AUTUMN && w.biome != TROPICAL && w.biome != DESERT) {
    for (int y=0;y<H;++y) for (int x=0;x<W;++x) {
      char c = next[y*W + x];
      if ((c==',' || c=='"' || c==';') && r.oneIn(180)) next[y*W + x] = 'a';
    }
  }
  if (s == WINTER && w.biome != TROPICAL && w.biome != DESERT) {
    for (int y=0;y<H;++y) for (int x=0;x<W;++x) {
      char c = next[y*W + x];
      if ((c=='f' || c=='+' || c=='&' || c=='!' || c=='"') && r.oneIn(28)) next[y*W + x] = '.';
      if (c==';' && r.oneIn(80)) next[y*W + x] = ',';
    }
  }
  if (w.biome == MEADOW || w.biome == WETLAND || w.biome == ALPINE) {
    for (int y=0;y<H;++y) for (int x=0;x<W;++x) {
      char c = next[y*W + x];
      if (c==';' || c=='"') {
        int neigh = countNeighborsTerrain(next, x, y, c);
        if (neigh >= 6 && r.oneIn(2)) next[y*W + x] = ',';
        if (neigh >= 7 && r.oneIn(2)) next[y*W + x] = '.';
      }
    }
  }
  w.terrain.swap(next);
...
static void stepEntities(World& w, Rng& r, Season s, int tick) {
  ...
  int totalEnt = 0;
  for (int i=0;i<W*H;++i) if (out[i] != ' ') totalEnt++;
  int maxEnt = (W*H)/18;
  if (totalEnt > maxEnt) {
    int toCull = totalEnt - maxEnt;
    for (int tries=0; tries < toCull*6; ++tries) {
      int i = r.i(0, W*H-1);
      if (out[i] != ' ') { out[i] = ' '; toCull--; if (toCull<=0) break; }
    }
  }

  
  // Desert: occasional scorpions and lizards/camels; keep overall density stable.
  if (w.biome==DESERT && scorpions < SCORPION_CAP && r.oneIn(28)) {
    for (int tries=0; tries<800; ++tries) {
      int x=r.i(0,W-1), y=r.i(0,H-1);
      if (!canSpawnAt(x,y)) continue;
      if (w.terrain[y][x]=='s' || w.terrain[y][x]=='.') { out[y][x]='z'; break; }
    }
  }
