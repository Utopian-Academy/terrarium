// --- Event-driven audio hooks (counts gathered during sim; consumed by audio tick)
struct SimAudioEvents {
  int floraGrows = 0;
  int floraDies  = 0;
  int agentDeaths = 0;
  int attacks    = 0;
...
static SimAudioEvents gAudioEv;
...
// --- Audio events: approximate flora growth/death from terrain changes ---
auto isFloraGlyph = [](char c)->bool {
  switch (c) {
    case ',': case ':': case ';': case '"': case '*': case '+':
    case 'f': case 'x': case 'd': case '$': case '!': case '&': case '#':
      return true;
    default: return false;
  }
};
int grows=0, deaths=0;
for (int yy=0; yy<H; ++yy) for (int xx=0; xx<W; ++xx) {
  char o = w.terrain[yy][xx];
  char n = next[yy][xx];
  if (!isFloraGlyph(o) && isFloraGlyph(n)) grows++;
  else if (isFloraGlyph(o) && !isFloraGlyph(n)) deaths++;
}
gAudioEv.floraGrows += grows;
gAudioEv.floraDeaths += deaths;
...
static void stepEntities(World& w, Rng& r, Season s, int tick) {
  ...
  if (doUpdate) {
    for (auto &a : w.agents) {
      // Baseline needs
      float dt = 0.1f; // ~100ms
      const float prevStress = a.stress;
      ...
      // Audio: emotional trigger when panic threshold is crossed.
      if (prevStress < 0.75f && a.stress >= 0.75f) gAudioEv.scares++;
    }

    // Cull dead agents
    const int _before = (int)w.agents.size();
    w.agents.erase(std::remove_if(w.agents.begin(), w.agents.end(),
      [&](const Agent& a){ return a.health <= 0.01f; }), w.agents.end());
    const int _after = (int)w.agents.size();
    if (_after < _before) gAudioEv.agentDeaths += (_before - _after);
  }
...
struct SimAudioEvents {
  int floraGrows = 0;     // '.' -> plant-ish
  int floraDeaths = 0;    // plant-ish -> '.'
  int attacks = 0;        // predator hit
  int scares = 0;         // stress threshold crossed
  int agentDeaths = 0;    // removed agents
};
// Accumulated during sim step; consumed by synthTickMusic() once per tick.
static SimAudioEvents gAudioEv;
...
// --- Event-driven audio (simulation events -> notes) ---
// We accumulate simple counters during the sim step (growth/death/attacks/panic),
// then emit short notes here with light rate-limiting.
// This intentionally replaces the old "timer-based clunk" generator.
struct PendingOff { int tickOff; int ch; int note; };
static PendingOff pend[64];
static int pendN = 0;

// Process scheduled note-offs
for (int i=0; i<pendN; ) {
  if (tick >= pend[i].tickOff) {
    synth.noteOff(pend[i].ch, pend[i].note, 0);
    pend[i] = pend[--pendN];
    continue;
  }
  ++i;
}
auto scheduleOff = [&](int ch, int note, int durTicks){
  if (pendN < (int)(sizeof(pend)/sizeof(pend[0]))) pend[pendN++] = { tick + durTicks, ch, note };
};

const int totalEv = gAudioEv.floraGrows + gAudioEv.floraDeaths + gAudioEv.attacks + gAudioEv.scares + gAudioEv.agentDeaths;
if (totalEv == 0) return;

// Gentle rate limiting per class (ticks are sim ticks, ~60/s in realtime mode)
static int lastGrowTick  = -999999;
static int lastDeathTick = -999999;
static int lastAtkTick   = -999999;
static int lastScareTick = -999999;

// Choose notes from simple scales (we can later map to your full scale/mode system)
auto pickPentMajor = [&]()->int { static const int iv[]={0,2,4,7,9}; return iv[r.i(0,4)]; };
auto pickPentMinor = [&]()->int { static const int iv[]={0,3,5,7,10}; return iv[r.i(0,4)]; };

// Flora growth: light bell-ish notes
if (gAudioEv.floraGrows > 0 && (tick - lastGrowTick) >= 2) {
  int vel = std::clamp(20 + gAudioEv.floraGrows * 3, 25, 110);
  int note = std::clamp(rootKey + 12 + pickPentMajor(), 0, 127);
  synth.noteOn(0, note, vel);
  scheduleOff(0, note, 10);
  lastGrowTick = tick;
}

// Flora death / agent death: lower, darker notes
if ((gAudioEv.floraDeaths + gAudioEv.agentDeaths) > 0 && (tick - lastDeathTick) >= 3) {
  int amt = gAudioEv.floraDeaths + gAudioEv.agentDeaths;
  int vel = std::clamp(25 + amt * 4, 30, 115);
  int note = std::clamp(rootKey - 12 + pickPentMinor(), 0, 127);
  synth.noteOn(0, note, vel);
  scheduleOff(0, note, 14);
  lastDeathTick = tick;
}

// Predator hits: short drum accents (channel 9)
if (gAudioEv.attacks > 0 && (tick - lastAtkTick) >= 1) {
  int vel = std::clamp(30 + gAudioEv.attacks * 8, 35, 120);
  int drum = 38; // snare-ish in GM
  synth.noteOn(9, drum, vel);
  scheduleOff(9, drum, 6);
  lastAtkTick = tick;
}

// Panic threshold crossed: bright high ping
if (gAudioEv.scares > 0 && (tick - lastScareTick) >= 4) {
  int vel = std::clamp(35 + gAudioEv.scares * 10, 45, 120);
  int note = std::clamp(rootKey + 24 + pickPentMajor(), 0, 127);
  synth.noteOn(0, note, vel);
  scheduleOff(0, note, 8);
  lastScareTick = tick;
}

// Consume
gAudioEv = SimAudioEvents{};
