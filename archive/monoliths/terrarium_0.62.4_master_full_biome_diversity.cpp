...
static constexpr int MAX_AGENTS = 50;
...
for (int y=0;y<H;++y) for (int x=0;x<W;++x) {
  ...
  for (int y=0; y<H; ++y) for (int x=0; x<W; ++x) {
    ...
    float chance = fd.grow * (0.6f + 0.8f*habitat) * 10.0f;
    if (r.u01() < chance) w.terrain[y][x] = (char)fd.glyph;
  }
    ...
    if (c=='.') {
      {
        ...
        float wet01 = std::clamp(wet / 8.0f, 0.0f, 1.0f);
        float habitat = 1.0f - std::fabs(wet01 - fd.wetPref);
        float pflora = fd.grow * (0.35f + 0.9f*habitat) * springBoost * rainBoost * altGrow;
        if (r.u01() < pflora) { next[y][x] = (char)fd.glyph; continue; }
      }
...
static void spawnInitialAgents(World& w, Rng& r){
  ...
  auto spawnSpecies = [&](uint8_t spec, int count){
    const FaunaDef &fd = g_faunaDefs[w.biome][spec];
    bool aquatic = isAquaticTraits(fd.traits);
    for (int i=0; i<count && (int)w.agents.size() < MAX_AGENTS; ++i){
  ...
  // Start small: two pairs of each "type" (aquatic, predator, herbivore, bird).
  auto pickSpeciesByTrait = [&](uint8_t mask, bool mustNotPred)->std::vector<uint8_t>{
    std::vector<uint8_t> out;
    for (int i=0;i<FAUNA_N;++i){
      const FaunaDef &fd = g_faunaDefs[w.biome][i];
      if ((fd.traits & mask) == 0) continue;
      if (mustNotPred && (fd.traits & FT_PRED)) continue;
      out.push_back((uint8_t)i);
    }
    if (out.size() > 1) std::shuffle(out.begin(), out.end(), r.rng);
    return out;
  };

  const int PAIRS_PER_SPEC = 2; // two pairs = 4 individuals
  const int SPEC_PER_TYPE = 2;

  // Aquatic
  {
    auto list = pickSpeciesByTrait(FT_AQUATIC, false);
    int n = std::min<int>(SPEC_PER_TYPE, (int)list.size());
    for (int i=0;i<n;++i) spawnSpecies(list[i], PAIRS_PER_SPEC * 2);
  }
  // Predator
  {
    auto list = pickSpeciesByTrait(FT_PRED, false);
    int n = std::min<int>(SPEC_PER_TYPE, (int)list.size());
    for (int i=0;i<n;++i) spawnSpecies(list[i], PAIRS_PER_SPEC * 2);
  }
  // Herbivore (non-pred)
  {
    auto list = pickSpeciesByTrait(FT_HERB, true);
    int n = std::min<int>(SPEC_PER_TYPE, (int)list.size());
    for (int i=0;i<n;++i) spawnSpecies(list[i], PAIRS_PER_SPEC * 2);
  }
  // Bird
  {
    auto list = pickSpeciesByTrait(FT_BIRD, false);
    int n = std::min<int>(SPEC_PER_TYPE, (int)list.size());
    for (int i=0;i<n;++i) spawnSpecies(list[i], PAIRS_PER_SPEC * 2);
  }

  // Fallback: if nothing spawned, seed a tiny random set.
  if (w.agents.empty()) {
    for (int i=0; i<8 && (int)w.agents.size() < MAX_AGENTS; ++i) {
      spawnSpecies(pickBiomeSpecies(w.biome, r), 1);
    }
  }
}
...
static void stepEntities(World& w, Rng& r, Season s, int tick) {
  ...
  if (doUpdate) {
    for (auto &a : w.agents) {
      ...
      if (tick > 420 && (int)w.agents.size() < MAX_AGENTS &&
          a.health > 0.75f && a.hunger < 0.45f && a.thirst < 0.45f &&
          r.u01() < (fd.reproduce * (0.6f + 0.6f*fd.social))) {
        int bx=a.x, by=a.y;
        moveRandom(r, bx, by);
        if (inBounds(bx,by)) {
          if (isAquaticTraits(a.traits)) {
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.62.5 | biome ") + biomeName(biome) +
