...
struct SynthOut {
  ...
  static void audioCb(void* userdata, Uint8* stream, int len) {
    SynthOut* self = (SynthOut*)userdata;
    if (!self || !self->synth) { std::memset(stream, 0, (size_t)len); return; }
    float* out = (float*)stream;
    int frames = len / (int)(sizeof(float)*2); // stereo float32
    fluid_synth_write_float(self->synth, frames, out, 0, 2, out, 1, 2);
  }
...
int main(int argc, char** argv) {
  ...
  for (int i=1; i<argc; ++i) {
    ...
    if (std::strcmp(argv[i], "--fullscreen")==0) startFull
...
for (int i=1; i<argc; ++i) {
  if (std::strcmp(argv[i], "--synth")==0) wantSynth = true;
...
";
      ...
      if (!synth.open(sf2Path, std::clamp(synthGain, 0.0f, 2.0f))) {
        std::cerr << "[synth] Failed to start synth (check FluidSynth build and --sf2 path).
";
      }
    }
  }
