...
static void spawnInitialAgents(World& w, Rng& r){
  ...
  else if (w.biome==WETLAND) dens = 1.25f;
  else if (w.biome==TROPICAL) dens = 1.35f;
  else if (w.biome==ALIEN) dens = 0.90f;

  auto spawn = [&](int count, char glyph, bool aquatic){
    for(int i=0;i<count;++i){
      for(int tries=0; tries<200; ++tries){
        ...
        Agent a; a.id=nextId++; a.x=x; a.y=y; a.glyph=glyph; a.species = pickBiomeSpecies(w.biome, r);
        a.hunger = r.u01()*0.4f;
        a.thirst = r.u01()*0.4f;
        a.stress = r.u01()*0.2f;
  ...
  int herb = (int)std::lround(RABBIT_CAP_BASE * 3.0f * dens);
  int pred = (int)std::lround(SNAKE_CAP_BASE  * 2.0f * dens);
  int bird = (int)std::lround(BIRD_CAP_BASE  * 1.0f * dens);
  int fish = (int)std::lround(FISH_CAP      * 2.0f * dens);
