...
static constexpr int FISH_CAP_BASE     = (W * H) / 260;
...
static constexpr int SCORPION_CAP    = (W * H) / 1500;
static constexpr int CRAB_CAP        = (W * H) / 1400;
static constexpr int DRAGONFLY_CAP   = (W * H) / 900;
static constexpr int JELLY_CAP       = (W * H) / 1200;
static constexpr int CRAWLER_CAP     = (W * H) / 1600;
static constexpr int ORB_CAP         = (W * H) / 2200;
...
static inline int countNeighborsTerrain(const std::vector<char>& g, int x, int y, char target) {
  int n=0;
  for (int dy=-1; dy<=1; ++dy) for (int dx=-1; dx<=1; ++dx) {
    if (!dx && !dy) continue;
    int nx=x+dx, ny=y+dy;
    if (!inBounds(nx,ny)) continue;
    if (g[ny*W + nx] == target) ++n;
  }
  return n;
}
...
static inline uint32_t biomeSalt(Biome b) {
  switch (b) {
    case MEADOW:   return 0x0A17C3u;
    case WETLAND:  return 0x055D1Bu;
    case ALPINE:   return 0x0C0FFEu;
    case DESERT:   return 0x0D3A5Eu;
    case TROPICAL: return 0x07A0F1u;
    case ALIEN:    return 0x0A11E1u;
    default: return 0x0BEEFu;
  }
}
static inline uint32_t seasonSalt(Season s) {
  switch (s) {
    case SPRING: return 0x13579u;
    case SUMMER: return 0x2468Au;
    case AUTUMN: return 0x0BADC0u;
    case WINTER: return 0x00F00Du;
    default: return 0x12345u;
  }
}
// Deterministic per-tile "species seed" for visual variety (does NOT change simulation rules).
static inline uint32_t speciesSeed(const World& w, int x, int y, Season s, uint32_t globalSeed) {
  return hash3((uint32_t)x, (uint32_t)y, globalSeed ^ biomeSalt(w.biome) ^ seasonSalt(s));
}
// Small integer variant 0..(n-1)
static inline int speciesVariant(const World& w, int x, int y, Season s, uint32_t globalSeed, int n) {
  if (n <= 1) return 0;
  uint32_t h = speciesSeed(w, x, y, s, globalSeed);
...
struct Weather {
  int cooldown = 0; // ticks until precipitation can start
...
// ---------------- OSC (minimal) ----------------
// Sends a small set of smoothed parameters to UDP OSC for Max/Ableton routing.
// Enable with: --osc <ip> <port>   (e.g., --osc 127.0.0.1 9000)
struct OscOut {
  bool enabled=false;
  int sock=-1;
  sockaddr_in addr{};
  ...
  static inline void pad4(std::vector<uint8_t>& b) { while (b.size() & 3u) b.push_back(0); }
  ...
  static inline void putF32(std::vector<uint8_t>& b, float f) {
    uint32_t u; static_assert(sizeof(float)==4,"");
    std::memcpy(&u, &f, 4);
    putU32(b, u);
  }
  ...
  void sendFloat(const char* path, float v) {
    if (!enabled) return;
    std::vector<uint8_t> b; b.reserve(128);
    putStr(b, path);
    putStr(b, ",f");
    putF32(b, v);
    ::sendto(sock, (const char*)b.data(), (int)b.size(), 0, (sockaddr*)&addr, (int)sizeof(addr));
  }
  void sendInt(const char* path, int v) {
    if (!enabled) return;
    std::vector<uint8_t> b; b.reserve(128);
    putStr(b, path);
    putStr(b, ",i");
...
struct ControlBus {
  // smoothed 0..1 controls
  float waterCoverage=0, waterDepth=0, waterFlow=0;
  float floraBiomass=0, floraDiversity=0, flowerBloom=0;
  float wind=0, clouds=0, rain=0;
  float seasonPhase=0;
  int seasonId=0;
  ...
  static inline float smooth(float cur, float tgt, float a) { return cur + (tgt-cur)*a; }

  void updateFromWorld(const World& w, int tick, int season, float sp) {
    (void)sp;
    // sample grid (cheap): stride sampling to avoid full scan cost
    int step = 3;
    int tot=0, water=0, waterSum=0;
    int flowSum=0;
    int veg=0, flowers=0;
    // diversity: count a few tile types
    int c_grass=0, c_shrub=0, c_tree=0, c_flower=0, c_other=0;

    for (int y=0; y<H; y+=step) for (int x=0; x<W; x+=step) {
      tot++;
      uint8_t wd = (uint8_t)w.water[y*W + x];
      if (wd) { water++; waterSum += wd; }
      char t = w.terrain[y][x];
      if (t==','||t=='"'||t==';'||t=='T'||t=='Y'||t=='P') veg++;
      if (t=='f'||t=='+'||t=='&'||t=='!' ) { flowers++; c_flower++; }
      if (t==','||t=='"') c_grass++;
      else if (t==';') c_shrub++;
      else if (t=='T'||t=='Y'||t=='P') c_tree++;
      else c_other++;

      // flow energy approx from local water gradient
      if (wd) {
        int here = (int)w.height[y][x] + (int)wd*8;
        int bestDrop = 0;
        for (int dy=-1; dy<=1; ++dy) for (int dx=-1; dx<=1; ++dx) {
          if(!dx&&!dy) continue;
          int nx=x+dx, ny=y+dy;
          if(!inBounds(nx,ny)) continue;
          int there = (int)w.height[ny][nx] + (int)waterAt(w,nx,ny)*8;
          bestDrop = std::max(bestDrop, here-there);
        }
        flowSum += bestDrop;
      }
    }

    float wc = tot? (float)water/(float)tot : 0.f;
    float md = water? (float)waterSum/(float)(water*7) : 0.f;
    float wf = water? std::min(1.f, (float)flowSum/(float)(water*18)) : 0.f;

    float fb = tot? (float)veg/(float)tot : 0.f;
    float bl = tot? (float)flowers/(float)tot : 0.f;

    // crude diversity (Shannon-ish) across 5 buckets
    float p[5] = {
      tot? (float)c_grass/tot : 0.f,
      tot? (float)c_shrub/tot : 0.f,
      tot? (float)c_tree/tot : 0.f,
      tot? (float)c_flower/tot : 0.f,
      tot? (float)c_other/tot : 0.f
    };
    float Hs=0.f;
    for(int i=0;i<5;++i) if(p[i]>1e-6f) Hs += -p[i]*std::log(p[i]);
    float div = std::min(1.f, Hs/1.6f);

    float windT = std::min(1.f, std::abs(w.wind.dx) + std::abs(w.wind.dy));
    float cloudsT = (w.weather.state==STORM ? 1.f : (w.weather.state==RAIN ? 0.7f : 0.25f)) * w.cloudOpacity;
    float rainT = (w.weather.state==STORM ? 1.f : (w.weather.state==RAIN ? 0.55f : 0.f));

    // smoothing
    waterCoverage = smooth(waterCoverage, wc, 0.05f);
    waterDepth    = smooth(waterDepth,    md, 0.04f);
    waterFlow     = smooth(waterFlow,     wf, 0.05f);
    floraBiomass  = smooth(floraBiomass,  fb, 0.03f);
    flowerBloom   = smooth(flowerBloom,   bl, 0.04f);
    floraDiversity= smooth(floraDiversity,div,0.02f);
    wind          = smooth(wind,          windT, 0.08f);
    clouds        = smooth(clouds,        cloudsT,0.06f);
    rain          = smooth(rain,          rainT, 0.10f);
...
static uint8_t sampleCloud(const Clouds& c, int x, int y) {
  float fx = (float)x / (float)W * (float)CW;
  float fy = (float)y / (float)H * (float)CH;
  int cx = (int)fx;
  int cy = (int)fy;
  cx = (cx + (int)c.offX) % CW;
  cy = (cy + (int)c.offY) % CH;
...
static void updateWeather(World& w, Rng& r, int tick) {
  if (w.weather.cooldown > 0) w.weather.cooldown--;
  w.weather.timer++;
...
static void stepSnow(World& w, Rng& r, Season s) {
  if (w.snow.empty()) return;
  if (w.biome == TROPICAL || w.biome == DESERT) {
    std::fill(w.snow.begin(), w.snow.end(), 0);
    return;
  }
  for (int y=0;y<H;++y) for (int x=0;x<W;++x) {
    int i = y*W + x;
    if (w.water[y*W + x] > 0) { w.snow[i] = 0; continue; }
    uint8_t sn = w.snow[i];
    if (s == WINTER) {
      int addChance = (w.weather.state==STORM ? 6 : (w.weather.state==RAIN ? 12 : 90));
      if (r.oneIn(addChance) && sn < 3) sn++;
      if (sn>0 && r.oneIn(1600)) sn = (uint8_t)std::max(0, (int)sn-1);
    } else if (s == SPRING) {
      if (sn>0 && r.oneIn(20)) sn--;
    } else if (s == SUMMER) {
      if (sn>0 && r.oneIn(9)) sn--;
    } else {
      if (sn>0 && r.oneIn(90)) sn--;
    }
    w.snow[i] = sn;
  }
}
...
static void seedWorld(World& w, Rng& r, Biome biome) {
  ...
  w.worldSeed = (uint32_t)r.i(0, 0x7fffffff);
w.biome = biome;
  w.targetBiome = biome;
  w.biomeFade = 0.0f;
  w.biomeFadeDir = 0;
  ...
  w.cloudOpacity = 1.0f;
  if (biome==TROPICAL) w.cloudOpacity = 0.32f; // clearer sky
  if (biome==DESERT)   w.cloudOpacity = 0.22f;
  if (biome==ALIEN)    w.cloudOpacity = 0.70f;
...
static void stepTerrain(World& w, Rng& r, Season s, int tick) {
  ...
  // Extra flora growth (kept gentle + biome-aware)
  if (w.biome == MEADOW || w.biome == WETLAND || w.biome == TROPICAL) {
    for (int y=1;y<H-1;++y) for (int x=1;x<W-1;++x) {
      char c0 = next[y][x];
      if (c0!='.' && c0!=',') continue;
      // detect nearby water
      bool nearW=false;
      for(int dy=-2;dy<=2 && !nearW;++dy) for(int dx=-2;dx<=2;++dx){
        int nx=x+dx, ny=y+dy;
        if(inBounds(nx,ny) && waterAt(w,nx,ny)>0) { nearW=true; break; }
      }
      if (nearW && r.oneIn(w.biome==TROPICAL ? 140 : 220)) next[y][x] = 'r'; // reeds
      else if (nearW && r.oneIn(520)) next[y][x] = 'M'; // moss
      else if (r.oneIn(520)) next[y][x] = 'v'; // fern
      else if (r.oneIn(680)) next[y][x] = 'c'; // clover
    }
  }

// AUTUMN/WINTER seasonal transforms + dieback + density cap
  if (s == AUTUMN && w.biome != TROPICAL && w.biome != DESERT) {
    for (int y=0;y<H;++y) for (int x=0;x<W;++x) {
      char c = next[y][x];
      if ((c==',' || c=='"' || c==';') && r.oneIn(180)) next[y][x] = 'a';
    }
  }
  if (s == WINTER && w.biome != TROPICAL && w.biome != DESERT) {
    for (int y=0;y<H;++y) for (int x=0;x<W;++x) {
      char c = next[y][x];
      if ((c=='f' || c=='+' || c=='&' || c=='!' || c=='"') && r.oneIn(28)) next[y][x] = '.';
      if (c==';' && r.oneIn(80)) next[y][x] = ',';
    }
  }
  if (w.biome == MEADOW || w.biome == WETLAND || w.biome == ALPINE) {
    for (int y=0;y<H;++y) for (int x=0;x<W;++x) {
      char c = next[y][x];
      if (c==';' || c=='"') {
        int neigh = countNeighborsTerrainGrid(next, x, y, c);
        if (neigh >= 6 && r.oneIn(2)) next[y][x] = ',';
        if (neigh >= 7 && r.oneIn(2)) next[y][x] = '.';
      }
    }
  }
  w.terrain.swap(next);
...
static void stepEntities(World& w, Rng& r, Season s, int tick) {
  ...
  // Hard cap total entities to prevent takeover
  int totalEnt = 0;
  for (int yy=0; yy<H; ++yy) for (int xx=0; xx<W; ++xx) if (out[yy][xx] != ' ') totalEnt++;
  int maxEnt = (W*H)/18;
  if (totalEnt > maxEnt) {
    int toCull = totalEnt - maxEnt;
    for (int tries=0; tries < toCull*8; ++tries) {
      int xx = r.i(0, W-1), yy = r.i(0, H-1);
      if (out[yy][xx] != ' ') { out[yy][xx] = ' '; toCull--; if (toCull<=0) break; }
    }
  }
...
// DESERT fauna spawns (simple)
  ...
  // Aquatic fish (subtle): swim in deeper water; capped so they don't take over.
  if (w.biome == TROPICAL || w.biome == WETLAND || w.biome == MEADOW) {
    int fish=0;
    for (int y=0;y<H;++y) for (int x=0;x<W;++x) {
      char e = out[y][x];
      if ((unsigned char)e==0x19 || (unsigned char)e==0x1A) fish++;
    }
    if (fish < FISH_CAP_BASE && r.oneIn(18)) {
      for (int tries=0; tries<800; ++tries) {
        int x=r.i(0,W-1), y=r.i(0,H-1);
        if (waterAt(w,x,y) >= 3 && out[y][x]==' ') {
          out[y][x] = (r.oneIn(2) ? (char)0x19 : (char)0x1A);
          break;
        }
      }
    }
    // simple swim: drift along flow bias (uses water gradient)
    for (int k=0; k<220; ++k) {
      int x=r.i(1,W-2), y=r.i(1,H-2);
      char e = out[y][x];
      if (!((unsigned char)e==0x19 || (unsigned char)e==0x1A)) continue;
      if (r.oneIn(3)) continue;
      // pick best neighbor downhill on water surface
      int bestDx=0,bestDy=0;
      int here = (int)w.height[y][x] + (int)waterAt(w,x,y)*8;
      int bestDrop=-999;
      for (int dy=-1; dy<=1; ++dy) for (int dx=-1; dx<=1; ++dx) {
        if(!dx&&!dy) continue;
        int nx=x+dx, ny=y+dy;
        if(!inBounds(nx,ny)) continue;
        if (waterAt(w,nx,ny) < 2) continue;
        if (out[ny][nx] != ' ') continue;
        int there = (int)w.height[ny][nx] + (int)waterAt(w,nx,ny)*8;
        int drop = here - there;
        if (drop > bestDrop) { bestDrop=drop; bestDx=dx; bestDy=dy; }
      }
      int nx=x+bestDx, ny=y+bestDy;
      if (bestDrop>-999 && inBounds(nx,ny) && out[ny][nx]==' ') {
        out[ny][nx]=e;
        out[y][x]=' ';
      }
    }
...
static const uint8_t* glyph8(char c) {
  ...
  // Seasonal glyphs
  static const uint8_t SNW1[8] = {0x00,0x00,0x10,0x00,0x00,0x04,0x00,0x00}; // \x16 light snow
  static const uint8_t SNW2[8] = {0x00,0x10,0x00,0x04,0x00,0x10,0x00,0x04}; // \x17
  static const uint8_t SNW3[8] = {0x44,0x00,0x10,0x44,0x00,0x10,0x44,0x00}; // \x18 heavy snow
  static const uint8_t AUTG[8] = {0x00,0x00,0x14,0x22,0x14,0x08,0x14,0x00}; // 'a' fallen leaves


  // Extra fauna glyphs (distinct archetypes)
  static const uint8_t SCOR[8] = {0x00,0x10,0x38,0x54,0x38,0x10,0x28,0x00}; // 'z' scorpion
  static const uint8_t CRAB[8] = {0x00,0x24,0x7E,0x3C,0x3C,0x7E,0x24,0x00}; // 'Z' crab
  static const uint8_t DRGN[8] = {0x00,0x24,0x18,0x7E,0x18,0x24,0x42,0x00}; // 'd' dragonfly
  static const uint8_t JELY[8] = {0x00,0x3C,0x7E,0x7E,0x3C,0x24,0x24,0x00}; // 'j' jellyfish
  static const uint8_t CRAW[8] = {0x00,0x3C,0x5A,0x3C,0x5A,0x3C,0x24,0x00}; // 'k' alien crawler
  static const uint8_t ORB0[8] = {0x00,0x18,0x3C,0x7E,0x7E,0x3C,0x18,0x00}; // 'o' glowing orb


  // Extra flora glyphs (variants)
  static const uint8_t FLO2[8]  = {0x00,0x24,0x18,0x7E,0x18,0x24,0x00,0x00}; // 'p'
  static const uint8_t FLO3[8]  = {0x00,0x18,0x3C,0x18,0x7E,0x18,0x24,0x00}; // 'q'
  static const uint8_t HERB[8]  = {0x00,0x10,0x38,0x54,0x10,0x38,0x10,0x00}; // 'h'
  static const uint8_t MOSS[8]  = {0x00,0x00,0x2A,0x54,0x2A,0x54,0x00,0x00}; // 'm'

  // Aquatic fauna
  static const uint8_t FISH1[8] = {0x00,0x00,0x18,0x3C,0x7E,0x3C,0x18,0x00}; // \x19
  static const uint8_t FISH2[8] = {0x00,0x00,0x10,0x38,0x7C,0x38,0x10,0x00}; // \x1A
  ...
  switch (c) {
    ...
    case '\x16': return SNW1; case '\x17': return SNW2; case '\x18': return SNW3;
    case '\x19': return FISH1; case '\x1A': return FISH2;
    case 'a': return AUTG;
    case 'z': return SCOR;
    case 'Z': return CRAB;
    case 'd': return DRGN;
    ...
    case 'k': return CRAW;
    case 'o': return ORB0;
    case 'p': return FLO2; case 'q': return FLO3; case 'h': return HERB; case 'm': return MOSS;
...
static RGB fgForChar(const World& w, char c, Season s, float sp, int tick, int x, int y) {
  ...
  else if ((unsigned char)c>=0x16 && (unsigned char)c<=0x18) { fg = {235,245,255}; }
  else if (c=='a') { fg = {190,140,80}; }
  else if (c=='z') { fg = {200,150,110}; }
  else if (c=='Z') { fg = {235,120,120}; }
  else if (c=='d') { fg = {200,240,200}; }
  else if (c=='j') { fg = {170,200,255}; }
  else if (c=='k') { fg = {170,255,190}; }
  else if (c=='o') { fg = {220,220,255}; }
  else if (c=='p') { fg = {245, 180, 220}; }
  else if (c=='q') { fg = {240, 240, 200}; }
  else if (c=='h') { fg = {180, 220, 160}; }
  else if (c=='m') { fg = {110, 150, 110}; }
  else if ((unsigned char)c==0x19 || (unsigned char)c==0x1A) { fg = {120, 210, 255}; }
