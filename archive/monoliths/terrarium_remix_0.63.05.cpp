...
// ===== Config =====
static constexpr const char* VERSION = "0.63.05";
...
static inline float habitatSuitability(const World& w, const SpeciesDef& sd, int x, int y, int tick){
  ...
  if (sd.aquatic) {
    ...
    if (shore) s += 0.08f;
    if (sd.name && std::strcmp(sd.name, "CRAB")==0) {
      s += shore ? 0.16f : -0.08f;
    } else if (sd.name && std::strcmp(sd.name, "TURTLE")==0) {
      if (water < 2.2f) s += 0.08f;
      if (season==SUMMER) s += 0.05f;
    } else if (sd.name && std::strcmp(sd.name, "EEL")==0) {
      if (water > 1.8f) s += 0.12f;
      if (shore) s -= 0.05f;
    } else if (sd.name && std::strcmp(sd.name, "FROG")==0) {
      s += shore ? 0.15f : -0.06f;
    ...
    if (sd.name && std::strcmp(sd.name, "GOAT")==0) {
      s += 0.12f * alt;
    } else if (sd.name && std::strcmp(sd.name, "DEER")==0) {
      s += 0.08f * (1.0f - alt);
    } else if (sd.name && std::strcmp(sd.name, "RABBIT")==0) {
      if (shore) s -= 0.05f;
      if (fert > 0.55f) s += 0.08f;
    } else if (sd.name && std::strcmp(sd.name, "HAWK")==0) {
      s += 0.14f * alt;
    } else if (sd.name && std::strcmp(sd.name, "BOAR")==0) {
      s += 0.10f * moist;
    }
