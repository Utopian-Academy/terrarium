...
//   g++ -O2 -std=c++17 terrarium_0.31.cpp -o terrarium_0.31 `sdl2-config --cflags --libs`
//
// Run examples:
//   ./terrarium_0.31 --biome meadow
//   ./terrarium_0.31 --biome tropical
//   ./terrarium_0.31 --windowed --biome tropical
//   ./terrarium_0.31 --fullscreen --biome meadow
//
// Controls:
//   SPACE pause/unpause
//   .     step (when paused)
...
static constexpr float VIVID_SAT = 1.65f;
static constexpr float VIVID_VAL = 1.12f;
...
static void stepWater(World& w, Rng& r) {
  ...
  if (w.weather.rainStrength > 0.05f) {
    int hits = (int)((W*H)/220 * w.weather.rainStrength);
...
static void stepTerrain(World& w, Rng& r, Season s, int tick) {
  ...
  for (int y=0; y<H; ++y) for (int x=0; x<W; ++x) {
    ...
    if (c=='.') {
      ...
      float p = 0.0032f * fert * w.bw.growRate * springBoost * rainBoost / winterSlow;
    ...
    if (c==',') {
      ...
      if (wet>0 && r.u01() < (0.005f * springBoost * rainBoost * w.bw.bloomRate)) {
        float t = r.u01();
...
static RGB baseBgFor(const World& w, int x, int y, int tick, Season s, float sp) {
  ...
  switch (t) {
    case '.': c={0,0,0}; break;
    case ',': c={ jit(26,2), jit(82,3), jit(26,2) }; break;
    case '"': c={ jit(30,2), jit(110,4), jit(30,2) }; break;
...
static RGB fgForChar(const World& w, char c, Season s, float sp, int tick) {
  RGB fg{245,245,245};
...
static void render(SDL_Renderer* ren, const Layout& L, World& w, GlyphCache& gc, int tick) {
  ...
  for (int y=0; y<H; ++y) {
    ...
    for (int x=0; x<W; ++x) {
      ...
      if (gt) {
        RGB fg = fgForChar(w, c, s, sp, tick);
...
int main(int argc, char** argv) {
  ...
  while (running) {
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.31 | biome ") + biomeName(biome) +
