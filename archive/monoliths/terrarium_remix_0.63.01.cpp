...
struct SynthOut {
  ...
  void setGain(float){}
  void listPresets(std::vector<std::string>& out){ out.clear(); }
#endif
};
...
static inline char waterFlowGlyph(const World& w, int x, int y, int tick){
  ...
  float vx = std::cos(ang) + wx + phase*0.2f + (-gy)*swirl;
  float vy = std::sin(ang) + wy - phase*0.15f + (gx)*swirl;
...
static void render(SDL_Renderer* ren, const Layout& L, World& w, GlyphCache& gcWorld, GlyphCache& gcText, int tick, bool showMenu, int menuPage, int menuSel){
  ...
  if (showMenu) {
    ...
    if (menuPage==0) {
      ...
      ty += 16;
      std::snprintf(buf, sizeof(buf), "PALETTE EDIT (%s)", g_biomes[world.biome].name);
      drawString(ren, gcText, tx, ty, buf, 200,200,200, 230, 1);
      ty += 12;
      drawString(ren, gcText, tx, ty, "UP/DOWN pick  LEFT/RIGHT channel  +/- edit", 170,170,180, 230, 1);
      ty += 12;
      menuSel = clampi(menuSel, 0, 8);
      ...
      if (e.type==SDL_KEYDOWN){
        switch(e.key.keysym.sym){
          ...
          case SDLK_UP: {
            ...
            else if (showMenu && menuPage==7) { menuSel = (menuSel + 8) % 9; }
          } break;
          case SDLK_DOWN: {
            ...
            else if (showMenu && menuPage==7) { menuSel = (menuSel + 1) % 9; }
          } break;
          case SDLK_LEFT:
