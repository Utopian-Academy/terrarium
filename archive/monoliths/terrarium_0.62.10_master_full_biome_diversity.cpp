...
static void updateModPool(World& w, int tick, int viewW, int viewH){
  ...
  float o0 = (g_modVal[20]*g_modVal[15]);                  // aquatic stress * ripples
  ...
  for (int y=0; y<H; ++y) {
    ...
    if (c=='.') {
      if (floraCount < MAX_FLORA) {
        ...
        float wet01 = std::clamp(wet / 8.0f, 0.0f, 1.0f);
        float habitat = 1.0f - std::fabs(wet01 - fd.wetPref);
        float pflora = fd.grow * (0.18f + 0.35f*habitat) * springBoost * rainBoost * altGrow;
...
for (int y=0; y<viewH; ++y) {
    ...
    for (int x=0; x<viewW; ++x) {
      ...
      // Bias to avoid full-screen shadows; emphasize smaller cloud patches.
      int shadow = (int)cloud - 130;
      if (shadow < 0) shadow = 0;
      shadow = std::min(255, shadow * 2);
      int layer = (int)cloud - 155;
      if (layer < 0) layer = 0;
      layer = std::min(255, layer * 2);

      applyCloudShadow(bg, (uint8_t)shadow);
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.62.11 | biome ") + biomeName(biome) +
