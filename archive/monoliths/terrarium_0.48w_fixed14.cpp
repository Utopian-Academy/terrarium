                                 ...
                                 const std::vector<MidiParam>& params)
{
  ...
  // You can override via MIDI param "Instr" (0..1), or let the sim occasionally switch it.
  ...
  // Auto-switch very rarely based on sim state.
  if (!instrExplicit && tick >= nextAutoChangeTick) {
    // Spread changes out; more likely during storms or when big sea life is around.
    ...
    int baseInterval = 60 * 6; // ~6 seconds at 60 TPS (rough)
    int jitter = r.irange(-120, 240);
    int boost = (world.weather.state==STORM ? -120 : 0) + (whales>0 ? -90 : 0) + (w01>0.55f ? -60 : 0);
    nextAutoChangeTick = tick + std::max(180, baseInterval + jitter + boost);

    // Choose a palette index with gentle bias: wetter -> more "Crystal", calmer -> Music Box/Marimba.
    float t = std::clamp((w01 - 0.25f) / 0.55f, 0.0f, 1.0f);
    float pick = std::clamp(t + (r.frand()*0.35f - 0.175f), 0.0f, 0.9999f);
    int prog = chooseFromPalette(pick);
  ...
  if (instrExplicit) {
    int prog = chooseFromPalette(instr01);
    if (prog != currentProgram) {
#ifdef USE_FLUIDSYNTH
#ifdef USE_FLUIDSYNTH
  #ifdef USE_FLUIDSYNTH
    ...
    // Default
    int prog = chooseFromPalette(0.10f);
#ifdef USE_FLUIDSYNTH
    fluid_synth_program_change(synth.synth, 0, prog);
#endif
    currentProgram = prog;
  ...
  const int N = 18; // slightly faster than before but still sparse
  if ((tick % N) != 0) return;
