...
// ===== Config =====
static constexpr const char* VERSION = "0.62.37d";
    ...
    for(int x=0;x<W;++x){
      if (w.terrain[y][x]=='*') {
        ...
        for(int dy=-1;dy<=1;++dy) for(int dx=-1;dx<=1;++dx){
          if (!dx && !dy) continue;
          int nx=x+dx, ny=y+dy; if(!inBounds(nx,ny)) continue;
          if (isVeg(w.terrain[ny][nx]) && r.u01() < spreadChance) next[ny][nx]='*';
        }
        // turn to ash
  ...
  for (size_t ai=0; ai<agentN; ++ai){
    ...
    for(int dy=-1;dy<=1;++dy) for(int dx=-1;dx<=1;++dx){
      ...
      if (sd.carnivore) {
        ...
        if (preyNear > 0) {
          score += (int)std::lround((2.0f + a.aggro + (sd.pack?1.0f:0.0f)) * preyNear);
        }
      }
    ...
    if (a.goalTTL <= 0 || !inBounds(a.goalX, a.goalY) || (a.x==a.goalX && a.y==a.goalY) || r.oneIn(6)) {
...
static void updateModPool(World& w, int tick){
  ...
  struct GridAcc {
    ...
    float meanAlt=0.f, meanAlt2=0.f, fertSum=0.f;
  };
...
static inline char waterFlowGlyph(const World& w, int x, int y, int tick){
  ...
  // bold swell direction (global waves)
  float sx = std::sin((float)tick*0.05f + x*0.03f + y*0.02f);
  float sy = std::cos((float)tick*0.04f + x*0.02f - y*0.03f);
  int sdx = (sx > 0.2f) ? 1 : (sx < -0.2f ? -1 : 0);
  int sdy = (sy > 0.2f) ? 1 : (sy < -0.2f ? -1 : 0);
  if (d >= 3 && (sdx != 0 || sdy != 0)) { bestDx = sdx; bestDy = sdy; }
  ...
  int dd = d;
  if (dd >= 4 && phase) dd = std::max(1, dd-1);
...
static RGB fgForChar(const World& w, char c, int x, int y, Season s){
  ...
  if ((c>='1' && c<='7') || (c>=0x01 && c<=0x15)) {
    ...
    float bright = (t < 0.4f) ? 1.10f : (t > 0.75f ? 0.85f : 1.0f);
...
static void render(SDL_Renderer* ren, const Layout& L, World& w, GlyphCache& gcWorld, GlyphCache& gcText, int tick, bool showMenu, int menuPage, int menuSel){
    ...
    for(int x=0;x<viewW;++x){
      ...
      if (w.water[wy][wx] > 0.2f) {
        ...
        for (int dy=-1; dy<=1 && !shore; ++dy) for (int dx=-1; dx<=1 && !shore; ++dx){
          ...
          if (!inBounds(nx,ny) || w.water[ny][nx] <= 0.2f) shore = true;
        }
  ...
  // Foreground particles (near-camera depth layer).
  {
    ...
    for (int i=0; i<n; ++i){
      int x = (int)((i*53 + tick*(4 + std::max(0,w.wind.strength))) % std::max(1, L.screenW));
      int y = (int)((i*97 + tick*(2 + std::max(0,w.wind.strength))) % std::max(1, L.simHpx));
  ...
  // on-screen legend
  if (!showMenu) {
    std::snprintf(buf, sizeof(buf), "M menu  B biome  R reseed  [ ] speed  WASD pan  Z/X zoom  G ghibli  H modhot  F5 preset  F6 save  F7 midi  F9 audio  F11 fullscreen  %s", VERSION);
    ...
    SDL_Event e; while(SDL_PollEvent(&e)){
      ...
      if (e.type==SDL_KEYDOWN){
        switch(e.key.keysym.sym){
          ...
          case SDLK_TAB: if (showMenu) menuPage = (menuPage + 1) % 7; break;
