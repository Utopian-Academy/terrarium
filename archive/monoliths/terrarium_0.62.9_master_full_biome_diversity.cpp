...
// Movement: very simple, but driven by needs and panic.
      ...
      if (a.flags & 1) {
        ...
        nx=a.x+bestDx; ny=a.y+bestDy;
        float moveBias = std::clamp(0.20f + 0.35f*fd.speed, 0.15f, 0.90f);
      ...
      } else if (isBirdTraits(a.traits) && r.u01() < 0.65f) {
      ...
      } else if (isAquaticTraits(a.traits) && r.u01() < 0.65f) {
        a.intent = INTENT_WANDER;
        ...
        a.intent = INTENT_WANDER;
        // wander
        float moveBias = std::clamp(0.25f + 0.35f*fd.speed, 0.20f, 0.95f);
...
default: break;
    ...
    Season s = seasonAt(tick);
    std::string title =
      std::string("Terrarium 0.62.10 | biome ") + biomeName(biome) +
