...
// ===== Config =====
static constexpr const char* VERSION = "0.64.00";
...
static inline bool planetScreenToWorld(const Layout& L, int sx, int sy, int& wx, int& wy){
  ...
  float radius = 0.33f * (float)std::min(L.screenW, L.simHpx) * std::clamp(g_planetZoom, 0.45f, 2.4f);
...
static void renderPlanetWorld(SDL_Renderer* ren, const Layout& L, World& w, int tick){
  ...
  for (int iy=0; iy<=LAT; ++iy){
    ...
    for (int ix=0; ix<=LON; ++ix){
      ...
      float persp = 0.72f + 0.34f * ((r.z + 1.0f) * 0.5f);
      v[k].sx = (int)std::lround(cx + r.x * radius * persp);
      v[k].sy = (int)std::lround(cy - r.y * radius * persp);
      v[k].z = r.z;
      v[k].col = col;
      v[k].vis = true;
    }
  ...
  for (int idx : order){
    const PV& p = v[idx];
    int sz = (p.z > 0.55f) ? 3 : (p.z > 0.15f ? 2 : 1);
